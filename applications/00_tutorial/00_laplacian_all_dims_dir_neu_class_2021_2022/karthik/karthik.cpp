...
// the true or false here allows us to assign dirichelt or Neumann boundary conditions with its corresponding value at its two boundary points
...
// the true or false here allows us to assign dirichelt or Neumann boundary conditions with its corresponding value at its four boundary faces
...
int main(int argc, char** args) {
  ...
  // ======= Mesh, files - BEGIN  ==================
   ...
   // Pushback means adding components to a vector from the end... add something from the bottom so that the previous indices remains the same.
...
//     ml_mesh.GenerateCoarseBoxMesh(0,2,0,0.,0.,0.,1.,0.,0.,EDGE3,fe_quad_rule.c_str());
  ...
  unsigned numberOfUniformLevels = 2/*4*/;
  ...
  // Erasing all the coarser levels and the finest one. It only leaves the finest one. All the intermediate ones that are kept in the process of refinement are earsed. $

  // ======= Mesh: Coarse erasing - END  ========================

  
  // ======= Solution - BEGIN ==================
  // solution object: Solution object defines every important function on the given mesh. Solution is a class that receives the input "mesh". It is also a class that contains both unknowns and given functions.
  ...
  // problem receives the information that there is multilevel solution object
  // ======= Solution - END ==================
  ...
  // Starting values of the field is assigned zero. In the beginning of the iterative solver. Says about how we first fill the vector
