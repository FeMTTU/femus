...
void AssembleBoussinesqAppoximation_AD(MultiLevelProblem &ml_prob, unsigned level, const unsigned &levelMax, const bool &assembleMatrix) {
  ...
  JacI.reserve((dim+2)*maxSize*(dim+2)*maxSize);
  ...
  for (int iel=msh->IS_Mts2Gmt_elem_offset[iproc]; iel < msh->IS_Mts2Gmt_elem_offset[iproc+1]; iel++) {
    ...
    for( unsigned i=0; i<nDofsT; i++) {
      ...
      unsigned solTDof = msh->GetMetisDof(iNode, solTType); // global to global mapping between solution node and solution dof
	...
	for(unsigned i=0; i<nDofsV; i++) {
	  ...
	  for(unsigned j=0; j<dim; j++) {
	    for( unsigned  k = 0; k < dim; k++ ){
	      NSV[k]   +=  ni * phiV_x[i*dim+j]*( gradSolV_gss[k][j] + gradSolV_gss[j][k] );
	...
	for (int jnode=0;jnode < nDofsTVP; jnode++){
	   JacI[ inode * nDofsTVP + jnode]=-Jac[ jnode * nDofsTVP + inode ];
	}
      } 
