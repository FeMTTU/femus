...
void AssembleU_AD(MultiLevelProblem &ml_prob, unsigned level, const unsigned &levelMax, const bool &assembleMatrix);
void AssembleV_AD(MultiLevelProblem &ml_prob, unsigned level, const unsigned &levelMax, const bool &assembleMatrix);
...
//   mlMsh.ReadCoarseMesh("./input/square_quad.neu","seventh",scalingFactor);
  ...
  for(unsigned i = 1; i < maxNumberOfMeshes; i++){ // loop on the mesh level
    ...
    for(unsigned j=0; j<3; j++){ // loop on the FE Order
      ...
      LinearImplicitSystem & systemU = mlProb.add_system < NonLinearImplicitSystem > ("PoissonU");
      LinearImplicitSystem & systemV = mlProb.add_system < NonLinearImplicitSystem > ("PoissonV");
...
void AssembleV_AD(MultiLevelProblem &ml_prob, unsigned level, const unsigned &levelMax, const bool &assembleMatrix) {
  ...
  for (int iel=msh->IS_Mts2Gmt_elem_offset[iproc]; iel < msh->IS_Mts2Gmt_elem_offset[iproc+1]; iel++) {
	...
	for(unsigned i=0; i<nDofs; i++) {
	  ...
	  double f =  exactSolValue * 4. *pi*pi*pi*pi * phi[i] ;  
...
void AssembleU_AD(MultiLevelProblem &ml_prob, unsigned level, const unsigned &levelMax, const bool &assembleMatrix) {
  ...
  for (int iel=msh->IS_Mts2Gmt_elem_offset[iproc]; iel < msh->IS_Mts2Gmt_elem_offset[iproc+1]; iel++) {
	...
	vector < adept::adouble > soluGauss_x(dim,0.);
	 ...
	 double solvGauss = 0;
	vector < double > solvGauss_x(dim,0.);
