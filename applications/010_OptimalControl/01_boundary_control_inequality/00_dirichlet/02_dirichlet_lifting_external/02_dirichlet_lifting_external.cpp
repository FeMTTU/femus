...
double Solution_set_initial_conditions(const MultiLevelProblem * ml_prob, const std::vector < double >& x, const char name[]) {
    ...
    if(!strcmp(name,"state")) {
        ...
        value = ctrl:: square_or_cube:: lifting_external < ctrl::GAMMA_CONTROL_LIST_OF_FACES_WITH_EXTREMES >::ControlDomainFlag_external_restriction(x);
    }
...
int main(int argc, char** args) {
  ...
  ml_prob.set_all_abstract_fe_AD_or_not();
    ...
    std::ostringstream  mesh_file_stream; mesh_file_stream << "../../../../../"  <<  Files::mesh_folder_path() << "00_salome/2d/rectangle_extended/extended_box_coarse.med";
