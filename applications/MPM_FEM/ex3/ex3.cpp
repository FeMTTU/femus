...
int main(int argc, char** args) {
  ...
  for(unsigned mat = 0; mat< 3; mat++){
    for(unsigned nl = 2; nl < 6; nl++) {
      ...
      double rho_MPM = 1000.;
      double nu_MPM = 0.4;
      double E_MPM = YM1[simulation];
  
      //initialize parameters for plate (FEM)
      double rho_FEM = 1000.;
      double nu_FEM = 0.4;
      double E_FEM = YM2[simulation];
      ...
      double yc = YC[simulation]; // 0.025 for 5 ref,  0.05 for 4 ref, 0.1 for 3 ref, 0.15 for 2 ref
