...
// unsigned nel = static_cast< unsigned > (pow (nel1d, dim));
  ...
  double L = (0.5 * (Xv[nve1d - 3] + Xv[nve1d - 3]) - Xv[0]);
...
//   //PrintLine ("./output/", XPprint, false, 0);
  ...
  std::vector < double > phiLP (gmpm[Np - 1]->_node.size(), 0.);
...
//               }
    ...
    phi.assign (gmpm[p]->_node.size(), 0.);
    dphi.resize (gmpm[p]->_node.size());
    ...
    for (unsigned i = 0; i <  gmpm[p]->_node.size(); i++) {
      ...
      if (weight[i] > 0.) {
        ...
        for (unsigned d = 0; d < dim; d++) {
          dphi[i][d] = weight[i] * sumdAlphaT[d] + dweight[i][d] * sumAlphaT; //+ weight[i] * sumAlphadT[d];
        }

        Ur[p] += phi[i] * P;
        dUr[p] += dphi[i][0] * P;

        fouti << gmpm[p]->_xp[0] << " " << phi[i] << std::endl;

        for (unsigned d = 0 ; d < dim; d++) {
          dfouti[0] << gmpm[p]->_xp[0] << " " << dphi[i][0] << std::endl;
        }

        ymin = (ymin < phi[i]) ? ymin : phi[i];
        ymax = (ymax > phi[i]) ? ymax : phi[i];
        phiSum += phi[i];
        for (unsigned d = 0 ; d < dim; d++) {

          dymin[d] = (dymin[d] < dphi[i][d]) ? dymin[d] : dphi[i][d];
          dymax[d] = (dymax[d] > dphi[i][d]) ? dymax[d] : dphi[i][d];
