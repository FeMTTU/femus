...
PetscErrorCode StokesWriteSolution(Stokes* s) {
  ...
  if (size == 1) {
    PetscViewer viewer;
    ierr = VecGetArrayRead(s->x, &array);
    CHKERRQ(ierr);
    ierr = PetscViewerASCIIOpen(PETSC_COMM_WORLD, "solution.dat", &viewer);
    CHKERRQ(ierr);
    ...
    ierr = VecRestoreArrayRead(s->x, &array);
    CHKERRQ(ierr);
...
PetscErrorCode StokesSetupIndexSets(Stokes* s) {
  ...
  PetscFunctionBeginUser;
  /* the two index sets */
  ierr = MatNestGetISs(s->A, s->isg, NULL);
  CHKERRQ(ierr);
...
PetscErrorCode StokesExactSolution(Stokes* s) {
  ...
  PetscFunctionBeginUser;
  /* velocity part */
  ierr = VecGetSubVector(s->y, s->isg[0], &y0);
  CHKERRQ(ierr);
  ierr = VecGetOwnershipRange(y0, &start, &end);
  CHKERRQ(ierr);
  ...
  for (row = start; row < end; row++) {
    ...
    ierr = VecSetValue(y0, row, val, INSERT_VALUES);
    CHKERRQ(ierr);
  }

  ierr = VecRestoreSubVector(s->y, s->isg[0], &y0);
  CHKERRQ(ierr);

  /* pressure part */
  ierr = VecGetSubVector(s->y, s->isg[1], &y1);
  CHKERRQ(ierr);
  ierr = VecGetOwnershipRange(y1, &start, &end);
  CHKERRQ(ierr);
  ...
  for (row = start; row < end; row++) {
    ...
    ierr = VecSetValue(y1, row, val, INSERT_VALUES);
    CHKERRQ(ierr);
  }

  ierr = VecRestoreSubVector(s->y, s->isg[1], &y1);
  CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
PetscErrorCode StokesRhs(Stokes* s) {
  ...
  CHKERRQ(ierr);
  ierr = VecGetOwnershipRange(b0, &start, &end);
  CHKERRQ(ierr);
  ...
  for (row = start; row < end; row++) {
    ...
    if (row < s->nx * s->ny) {
      ierr = StokesRhsMomX(s, i, j, &val);
      CHKERRQ(ierr);
    } else if (row < 2 * s->nx * s->ny) {
      ierr = StokesRhsMomY(s, i, j, &val);
      CHKERRQ(ierr);
    }

    ierr = VecSetValue(b0, row, val, INSERT_VALUES);
    CHKERRQ(ierr);
  }
  ...
  CHKERRQ(ierr);
  ierr = VecGetOwnershipRange(b1, &start, &end);
  CHKERRQ(ierr);

  for (row = start; row < end; row++) {
    ierr = StokesGetPosition(s, row, &i, &j);
    CHKERRQ(ierr);
    ierr = StokesRhsMass(s, i, j, &val);
    CHKERRQ(ierr);
    ierr = VecSetValue(b1, row, val, INSERT_VALUES);
    CHKERRQ(ierr);
  }
...
PetscErrorCode StokesSetupMatBlock10(Stokes* s) {
  ...
  ierr = MatTranspose(s->subA[1], MAT_INITIAL_MATRIX, &s->subA[2]);
  CHKERRQ(ierr);
  ierr = MatScale(s->subA[2], -1.0);
  CHKERRQ(ierr);
  ierr = MatSetOptionsPrefix(s->subA[2], "a10_");
  CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
PetscErrorCode StokesSetupMatBlock11(Stokes* s) {
  ...
  ierr = MatCreate(PETSC_COMM_WORLD, &s->subA[3]);
  CHKERRQ(ierr);
  ierr = MatSetOptionsPrefix(s->subA[3], "a11_");
  CHKERRQ(ierr);
  ierr = MatSetSizes(s->subA[3], PETSC_DECIDE, PETSC_DECIDE, s->nx * s->ny, s->nx * s->ny);
  CHKERRQ(ierr);
  ierr = MatSetType(s->subA[3], MATMPIAIJ);
  CHKERRQ(ierr);
  ierr = MatMPIAIJSetPreallocation(s->subA[3], 0, NULL, 0, NULL);
  CHKERRQ(ierr);
  ierr = MatAssemblyBegin(s->subA[3], MAT_FINAL_ASSEMBLY);
  CHKERRQ(ierr);
  ierr = MatAssemblyEnd(s->subA[3], MAT_FINAL_ASSEMBLY);
  CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
PetscErrorCode StokesSetupApproxSchur(Stokes* s) {
  ...
  CHKERRQ(ierr);
  ierr = VecSetSizes(diag, PETSC_DECIDE, 2 * s->nx * s->ny);
  CHKERRQ(ierr);
  ierr = VecSetType(diag, VECMPI);
  CHKERRQ(ierr);
  ...
  ierr = MatMatMult(s->subA[2], s->subA[1], MAT_INITIAL_MATRIX, PETSC_DEFAULT, &s->myS);
  CHKERRQ(ierr);
  ierr = MatScale(s->myS, -1.0);
  CHKERRQ(ierr);
...
PetscErrorCode StokesSetupMatrix(Stokes* s) {
  ...
  PetscFunctionBeginUser;
  ierr = StokesSetupMatBlock00(s);
  CHKERRQ(ierr);
  ierr = StokesSetupMatBlock01(s);
  CHKERRQ(ierr);
  ierr = StokesSetupMatBlock10(s);
  CHKERRQ(ierr);
  ierr = StokesSetupMatBlock11(s);
  CHKERRQ(ierr);
  ierr = MatCreateNest(PETSC_COMM_WORLD, 2, NULL, 2, NULL, s->subA, &s->A);
  CHKERRQ(ierr);
  ierr = StokesSetupApproxSchur(s);
  CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
//   ierr     = PetscOptionsHasName(NULL, "-user_pc", &s.userPC);
//   CHKERRQ(ierr);
//   ierr     = PetscOptionsHasName(NULL, "-user_ksp", &s.userKSP);
//   CHKERRQ(ierr);
// 
//   ierr = StokesSetupMatrix(&s);
//   CHKERRQ(ierr);
//   ierr = StokesSetupIndexSets(&s);
//   CHKERRQ(ierr);
//   ierr = StokesSetupVectors(&s);
//   CHKERRQ(ierr);
// 
//   ierr = KSPCreate(PETSC_COMM_WORLD, &ksp);
//   CHKERRQ(ierr);
//   ierr = KSPSetOperators(ksp, s.A, s.A);
//   CHKERRQ(ierr);
//   ierr = KSPSetFromOptions(ksp);
//   CHKERRQ(ierr);
//   ierr = StokesSetupPC(&s, ksp);
//   CHKERRQ(ierr);
//   ierr = KSPSolve(ksp, s.b, s.x);
//   CHKERRQ(ierr);
// 
//   /* don't trust, verify! */
//   ierr = StokesCalcResidual(&s);
//   CHKERRQ(ierr);
//   ierr = StokesCalcError(&s);
//   CHKERRQ(ierr);
//   ierr = StokesWriteSolution(&s);
//   CHKERRQ(ierr);
// 
//   ierr = KSPDestroy(&ksp);
//   CHKERRQ(ierr);
//   ierr = MatDestroy(&s.subA[0]);
//   CHKERRQ(ierr);
//   ierr = MatDestroy(&s.subA[1]);
//   CHKERRQ(ierr);
//   ierr = MatDestroy(&s.subA[2]);
//   CHKERRQ(ierr);
//   ierr = MatDestroy(&s.subA[3]);
//   CHKERRQ(ierr);
//   ierr = MatDestroy(&s.A);
//   CHKERRQ(ierr);
//   ierr = VecDestroy(&s.x);
...
=======
  ...
  s.nx     = 4;
  s.ny     = 6;
  ierr     = PetscOptionsGetInt(NULL,NULL, "-nx", &s.nx, NULL);
  CHKERRQ(ierr);
  ierr     = PetscOptionsGetInt(NULL, NULL,"-ny", &s.ny, NULL);
  CHKERRQ(ierr);
  s.hx     = 2.0 / s.nx;
  s.hy     = 1.0 / s.ny;
  s.userPC = s.userKSP = PETSC_FALSE;
  ierr     = PetscOptionsHasName(NULL,NULL, "-user_pc", &s.userPC);
  CHKERRQ(ierr);
  ierr     = PetscOptionsHasName(NULL,NULL, "-user_ksp", &s.userKSP);
  CHKERRQ(ierr);
