...
void GetStochasticData(std::vector <double>& alphas) {
  ...
  if(totMoments <= 0) {
    ...
    std::vector < std::vector <unsigned> > Tp;
    myuq.ComputeTensorProductSet(Tp, numberOfQuadraturePoints, numberOfEigPairs);
    ...
    myuq.EvaluateMultivariateHermitePoly(MultivariateHermitePoly, MultivariateHermiteQuadratureWeights,      
...
//     varianceQoI = moments[1] - meanQoI * meanQoI;
    ...
    for(unsigned m = 0; m < numberOfSamples; m++) {
      ...
      myuq.EvaluateHermitePolyHistogram(HermitePolyHistogram, pIndex, samplePoints,numberOfEigPairs);
