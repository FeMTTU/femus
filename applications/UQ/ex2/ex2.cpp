...
void GetEigenPair(MultiLevelProblem& ml_prob);
...
void GetEigenPair(MultiLevelProblem& ml_prob)
{
  ...
  int MM_size = msh->_dofOffset[soluType][nprocs];
  int MM_local_size = msh->_dofOffset[soluType][iproc + 1] - msh->_dofOffset[soluType][iproc];
  ...
  for (int iel = msh->_elementOffset[iproc]; iel < msh->_elementOffset[iproc + 1]; iel++) {
    ...
    // *** Gauss point loop ***
    for (unsigned ig = 0; ig < msh->_finiteElement[ielGeom1][soluType]->GetGaussPointNumber(); ig++) {
      // *** get gauss point weight, test function and test function partial derivatives ***
      msh->_finiteElement[ielGeom1][soluType]->Jacobian(x1, ig, weight1, phi1, phi_x, *nullDoublePointer);
      ...
      for (unsigned i = 0; i < nDof1; i++) {
        ...
        for (int jel = msh->_elementOffset[iproc]; jel < msh->_elementOffset[iproc + 1]; jel++) {
          ...
          unsigned nDof2  = msh->GetElementDofNumber(jel, soluType);    // number of solution element dofs
          ...
          for (unsigned jg = 0; jg < msh->_finiteElement[ielGeom2][soluType]->GetGaussPointNumber(); jg++) {
            // *** get gauss point weight, test function and test function partial derivatives ***
            msh->_finiteElement[ielGeom2][soluType]->Jacobian(x2, jg, weight2, phi2, phi_x, *nullDoublePointer);
