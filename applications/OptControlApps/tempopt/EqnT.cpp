...
//======= THIRD ROW (ADJOINT) ===================================
	 ...
	 if (i < _eqnmap._elem_type[vb][T4_ord]->GetNDofs()) { currelem._FeM[vb](ip3) +=  currelem._bc_eldofs[vb][ip3]*dtxJxW_g*(currgp._phi_ndsQLVB_g[vb][T4_ord][i]) + (1-currelem._bc_eldofs[vb][ip3])*detb*1300.;
...
// 	    int jp3 = j + 3*Tempold._ndof[vb];
	   if (i < _eqnmap._elem_type[vb][T4_ord]->GetNDofs() ) currelem._KeM[vb](ip3,ip3) += currelem._bc_eldofs[vb][ip3]*dtxJxW_g*(currgp._phi_ndsQLVB_g[vb][ T4_ord ][/*j*/i]*currgp._phi_ndsQLVB_g[vb][ T4_ord ][i]);   
#endif
