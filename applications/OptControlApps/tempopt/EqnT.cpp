...
//  QfluxDOTn>0: energy flows outside (cooling)  QfluxDOTn<0: energy flows inside (heating)
  ...
  /// b) Element  Loop over the volume (n_elem)
   const uint el_ngauss = _eqnmap._qrule[vb]._NoGaussVB;
...
//======= THIRD ROW (ADJOINT) ===================================
	 ...
	 if (i < _AbstractFE[VV][T4_ord]->_myelems[vb]->GetNDofs()) { currelem._FeM[vb](ip3) +=  currelem._bc_eldofs[vb][ip3]*dtxJxW_g*(currgp._phi_ndsQLVB_g[vb][T4_ord][i]) + (1-currelem._bc_eldofs[vb][ip3])*detb*1300.;
...
// 	    int jp3 = j + 3*Tempold._ndof[vb];
	   if (i < _AbstractFE[VV][T4_ord]->_myelems[vb]->GetNDofs() ) currelem._KeM[vb](ip3,ip3) += currelem._bc_eldofs[vb][ip3]*dtxJxW_g*(currgp._phi_ndsQLVB_g[vb][ T4_ord ][/*j*/i]*currgp._phi_ndsQLVB_g[vb][ T4_ord ][i]);   
#endif
