...
PetscErrorCode StokesSetupMatFp(Stokes*); /*setup the Fp laplace*/
...
//*...create A10 A01 Fp^(-1) preconditioner 
PetscErrorCode StokesSetupApproxSchur(Stokes *s)
{
  ...
  ierr = VecSetSizes(diag,PETSC_DECIDE,s->nx*s->ny);CHKERRQ(ierr);
  ierr = VecSetType(diag,VECMPI);CHKERRQ(ierr);
  ierr = MatGetDiagonal(s->Fp,diag);
  ierr = VecReciprocal(diag);

  /* compute: - A10 A01 diag(Fp)^(-1)*/
  ierr = MatDiagonalScale(s->subA[1],NULL,diag); /* (*warning* overwrites subA[1]) */
  ...
  /* restore A10 */
  ierr = MatGetDiagonal(s->Fp,diag);
  ierr = MatDiagonalScale(s->subA[1],NULL,diag);
...
PetscErrorCode StokesSetupMatrix(Stokes *s)
{
  ...
  ierr = StokesSetupMatFp(s);CHKERRQ(ierr);
