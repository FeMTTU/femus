...
bool SetBoundaryCondition(const std::vector < double >& x, const char SolName[], double& value, const int facename, const double time) {
  ...
  if (!strcmp(SolName, "T0")) {
    ...
    if (facename == 2 || facename == 3) { // Dirichlet 0
      value = 0.;
      if(x[0] < 0.00001  || x[0] > 0.99999 || x[1] < 0.5)
	value = 1.;  
    }
  }   
  else if (!strcmp(SolName, "U")) {
    if (facename == 1 ) { // Dirichlet 0
      value = 0.;  
    }
    else if (facename == 2 ) { // Dirichlet 0
      value = 0.;
    } 
    else if (facename == 3) {
...
//   FieldSplitTree FS_NST( JACOBI, FIELDSPLIT_PRECOND, FS2, "Benard");
  ...
  // else if( precType == ILU) system.SetMgSmoother(GMRES_SMOOTHER );
  ...
  NonLinearImplicitSystem& system2 = mlProb.add_system < NonLinearImplicitSystem > ("T");
  ...
  //if ( precType == FS_VTp || precType == FS_TVp ) system.SetFieldSplitTree(&FS_NST);
  system2.SetTolerances(1.e-5, 1.e-20, 1.e+50, 20, 20);
...
void AssembleTemperature(MultiLevelProblem& ml_prob){
  ...
  //  extract pointers to the several objects that we are going to use
  NonLinearImplicitSystem* mlPdeSys   = &ml_prob.get_system<NonLinearImplicitSystem> ("T");   // pointer to the linear implicit system named "Poisson"
  const unsigned level = mlPdeSys->GetLevelToAssemble();
