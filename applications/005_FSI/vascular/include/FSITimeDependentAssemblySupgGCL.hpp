...
//   double theta[3] = {0, 1. , 0.};
  ...
  void FSITimeDependentAssemblySupgNew2( MultiLevelProblem& ml_prob )
  {
    ...
    std::vector < std::vector < adept::adouble > > GradSolVAROld( dim );
    std::vector < std::vector < adept::adouble > > GradSolVARNew( dim );
    std::vector < std::vector < adept::adouble > > GradMeshVelOld( dim );
            ...
            for ( unsigned tip = 1; tip <= TimeIntPoints; tip++ ) {
              ...
              if ( !ml_sol->GetBdcFunction()( xx, "P", tau, face, time + ( -1. + s[tip] ) * dt ) && tau != 0. ) {
                ...
                for ( unsigned i = 0; i < nve; i++ ) {
                  ...
                  for ( unsigned idim = 0; idim < dim; idim++ ) {
                    vx_face[idim][i]    = ( *mymsh->GetTopology()->_Sol[idim] )( idof ) +
