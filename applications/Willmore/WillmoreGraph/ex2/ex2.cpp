  ...
  for (int iel = msh->GetElementOffset(iproc); iel < msh->GetElementOffset(iproc + 1); iel++) {
    ...
    for (unsigned ig = 0; ig < msh->_finiteElement[ielGeom][soluType]->GetGaussPointNumber(); ig++) {
      ...
      for (unsigned i = 0; i < nDofs; i++) {
        ...
        aResW[i] += (2.*solWGauss / A * phi[i] - nonLinearLaplaceU) * weight;
        aResu[i] += nonLinearLaplaceW * weight;

      } // end phi_i loop
    } // end gauss point loop

    //Neumann Boundary consition
    double tau = 0.;
    ...
    for (unsigned jface = 0; jface < msh->GetElementFaceNumber (iel); jface++) {
      ...
      if (el->GetFaceElementIndex (iel, jface) < 0) {
        unsigned int faceIndex =  el->GetBoundaryIndex (iel, jface);

        if (!mlSol->GetBdcFunction() (xx, "W", tau, faceIndex, 0.) && tau != 0.) {
          unsigned nve = msh->GetElementFaceDofNumber (iel, jface, solWType);
          const unsigned felt = msh->GetElementFaceType (iel, jface);

          for (unsigned i = 0; i < nve; i++) {
            unsigned int ilocal = msh->GetLocalFaceVertexIndex (iel, jface, i);
            unsigned inode = msh->GetSolutionDof (ilocal, iel, 2);

            for (unsigned idim = 0; idim < dim; idim++) {
              x[idim][i] = (*msh->GetTopology()->_Sol[idim]) (inode);
            }
          }

          for (unsigned igs = 0; igs < msh->_finiteElement[felt][solWType]->GetGaussPointNumber(); igs++) {
            msh->_finiteElement[felt][solWType]->JacobianSur (x, igs, weight, phi, phi_x, normal);
...
void GetExactSolutionGradientTorus (const std::vector < double >& x, std::vector < double >& solGrad) {
