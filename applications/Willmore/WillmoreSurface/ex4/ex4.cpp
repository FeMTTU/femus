/* This example details the implementation of the Willmore flow
 *   algorithm, applied to the blood cell example of Bhagya.
 *
 *   System0 AssembleInit computes the initial curvatures given mesh positions.
 *   System AssembleBloodCell solves the flow equations.
...
int main (int argc, char** args) {
  ...
  system.SetAssembleFunction (AssembleBloodCell);
...
void AssembleBloodCell (MultiLevelProblem& ml_prob) {
  ...
  for (int iel = msh->_elementOffset[iproc]; iel < msh->_elementOffset[iproc + 1]; iel++) {
    ...
    unsigned sizeAll = DIM * (nxDofs + nYDofs) + volumeConstraint + areaConstraint;
    ...
    for (unsigned ig = 0; ig < msh->_finiteElement[ielGeom][solxType]->GetGaussPointNumber(); ig++) {
      ...
      adept::adouble multiplier = (YdotN - 2 * c0) / (2 * YdotN);
      ...
      for (unsigned K = 0; K < DIM; K++) {
        for (unsigned i = 0; i < nxDofs; i++) {
          ...
          aResx[K][i] += (solYNewg[K] * phix[i] + term1) * Area;
        }
        ...
        for (unsigned i = 0; i < nYDofs; i++) {
          ...
          // Blood cell equation
          aResY[K][i] += ( ( (solLambda1 /*- YdotN * solLambda2*/) * normal[K] + (solxNewg[K] - solxOldg[K])  / dt) * phiY[i]
                           + solLambda2 * term1
                           + ( (kc / 4) * Hminc0sqrd + gamma1 ) * term1
                           + kc * multiplier * term3
                           - kc * multiplier * term0
                           // - kc * multiplier * ( YdotY + term2 ) * phiY_Xtan[K][i]
                           - kc * multiplier * ( YdotY + term2 ) * term1
                         ) * Area;
        }
      ...
      energy += ( (kc / 4) * Hminc0sqrd.value() + gamma1 ) * Area.value();
