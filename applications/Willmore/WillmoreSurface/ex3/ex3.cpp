...
// Area constraint is ALWAYS FALSE for MCF.
...
void AssembleO2ConformalMinimization (MultiLevelProblem&);
...
int main (int argc, char** args) {
  ...
  //mlMsh.ReadCoarseMesh ("../input/fertility.med", "seventh", scalingFactor);
...
void AssembleMCF (MultiLevelProblem& ml_prob) {
  ...
  for (int iel = msh->_elementOffset[iproc];
    ...
    for (unsigned ig = 0; ig < msh->
      ...
      energy += sumP3.value() / pow (2.,P[0]) * Area.value();
