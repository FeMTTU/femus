...
//   system.SetAbsoluteLinearConvergenceTolerance ( 1.e-13 );
  ...
  if (twoPressure){
    ...
    //}
    //else{
      system.SetTolerances(1.e-10, 1.e-12, 1.e+50, 40, 40);//(1.e-10, 1.e-50, 1.e+50, 40, 40)
    //}
  }
  else{
...
//   }
  ...
  if(iproc == 0){
    ...
    sprintf(stdOutputName, "stdoutput_level%d_nprocs%d_AAA3D.txt",numberOfUniformRefinedMeshes, nprocs);
...
void PrintConvergenceInfo(char *stdOutfile, const unsigned &level, const int &nprocs){
  ...
  sprintf(outFileName, "AAA3D_convergence_level%d_nprocs%d.txt",level, nprocs);
