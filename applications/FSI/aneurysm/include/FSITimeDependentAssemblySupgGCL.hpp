...
namespace femus
{
  ...
  begin:
		  ...
		  vx_face_old[idim][i] = (*mymsh->_topology->_Sol[idim])(idof) + Soli_old[indexVAR[idim]][ilocal];
              ...
              for (unsigned igs = 0; igs < mymsh->_finiteElement[felt][SolType2]->GetGaussPointNumber(); igs++) {
                ...
                for (unsigned i = 0; i < nve; i++) {
                  ...
                  for (unsigned idim = 0; idim < dim; idim++) {
                    if ((!solidmark[ilocal])) {
                      aRhs[indexVAR[dim + idim]][ilocal]   += (0.5 * value + (1. - 0.5) * value_old) * normal[idim];
                    }
                    else {   //if interface node it goes to solid
                      aRhs[indexVAR[idim]][ilocal]   += (0.5 * value + (1. - 0.5) * value_old) * normal[idim];
                    }
                  }
                }
              }
	...
	for (unsigned tip = 0; tip < TimeIntPointsp2; tip++){
	  vector <vector < adept::adouble> > vxs(dim);
	   
	  for (unsigned idim = 0; idim < dim; idim++) { 
	    vxs[idim].resize(nve);
	    for (int j = 0; j < nve; j++) {
	      vxs[idim][j] = vx_old[idim][j] * (1. - s[tip]) +  vx[idim][j] * s[tip];
	    }
	  }
	  mymsh->_finiteElement[ielt][SolType2]->Jacobian(vxs, ig, Weight, phi, gradphi, nablaphi);
	  
	  if (tip == (TimeIntPointsp2 - 1) && flag_mat != 4) {
	  ...
	  // store velocity and displacenet solution, gradient and laplace
	  for (int i = 0; i < nBlocks * dim; i++) {
	    SolVAR[i] = 0.;
	    for (int j = 0; j < dim; j++) {
	      GradSolVAR[i][j] = 0.;
	    ...
	    for (unsigned inode = 0; inode < nve; inode++) {
	      SolVAR[i] += phi[inode] * ( s[tip] * Soli[indexVAR[i]][inode] + (1. - s[tip]) * Soli_old[indexVAR[i]][inode] );
              for (int j = 0; j < dim; j++) {
	  ...
	  // store gauss point coordinates (old and new), grid velocity and grad (old)
	  if ( tip == 0 ){
	    // mesh velocity OLD
	    for (int i = 0; i < dim; i++) {
	      vx_ig[i]=0.;
	    ...
	    // store displacenet solution old and New
	    for (int i = 0; i < dim * nBlocks; i++) {
	      SolVAROld[i] = 0.;
	      SolVARNew[i] = 0.;
	      for (unsigned inode = 0; inode < nve; inode++) {
		SolVAROld[i] += phi[inode] * Soli_old[indexVAR[i]][inode];
		SolVARNew[i] += phi[inode] * Soli[indexVAR[i]][inode];
	      }
	    }
	  }
	  ...
	  // get mesh velocity at s
	  vector < adept::adouble > meshVel(dim);
	  vector < vector < adept::adouble > > GradMeshVel(dim);
	  for (unsigned i = 0; i < dim; i++) {
	    meshVel[i] = meshVel_old[i] + 2. * s[tip] * ( ( SolVARNew[i] - SolVAROld[i] )/dt - meshVel_old[i]);
	    GradMeshVel[i].resize(dim);
	    for (unsigned j = 0; j < dim; j++) {	    
	      GradMeshVel[i][j] = GradMeshVel_old[i][j] + 2. * s[tip] * ( (GradSolVARNew[i][j] - GradSolVAROld[i][j])/dt - GradMeshVel_old[i][j]);
	    }
	  }
	  ...
	  if (flag_mat != 4 ) {
	    ...
	    for (unsigned i = 0; i < nve; i++) {
	      //BEGIN redidual Laplacian ALE map in the reference domain
              if (tip == ( TimeIntPointsp2 - 1) && !solidmark[i]) {
		...
		adept::adouble LapvelVAR[3] = {0., 0., 0.};
		    ...
		    Lapdisp[idim]     += (GradSolVAR[idim][jdim] + GradSolVAR[jdim][idim]) * gradphi[i * dim + jdim];
                ...
                for (int idim = 0; idim < dim; idim++) {
                  ...
                  adept::adouble timeDerivative = - ( -( tip == 0) + (tip == ( TimeIntPointsp2 - 1) ) ) * SolVAR[dim + idim] * (phi[i] + phiSupg[i]) * Weight/dt;
                                                    

                  adept::adouble value =  theta[tip] * (
	  ...
	  //BEGIN SOLID ASSEMBLY ============
		...
		if(tip == 0){
		  for (int idim = 0; idim < dim; idim++) {
		    aRhs[indexVAR[dim + idim]][i] +=  - phi[i] * ( (-SolVARNew[idim] + SolVAROld[idim]) / dt +
								  0.5*( SolVARNew[dim + idim] + 0.5 * SolVAROld[dim + idim])
								 ) * Weight_hat;
		  }
		}
		...
		for (int idim = 0; idim < dim; idim++) {
		  ...
		  adept::adouble timeDerivative = -  ( -( tip == 0) + (tip == (TimeIntPointsp2 - 1) ) ) * rhos * SolVAR[dim + idim] * phi[i] * Weight / dt;
		  
		  adept::adouble value =  theta[tip] * (rhos * phi[i] * _gravity[idim]      // body force
