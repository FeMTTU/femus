  ...
  void FSITimeDependentAssemblySupgNew(MultiLevelProblem & ml_prob)
  {
    ...
    for (int iel = mymsh->_elementOffset[iproc]; iel < mymsh->_elementOffset[iproc + 1]; iel++) {
      ...
      for (int j = 0; j < nve; j++) {
        unsigned idof = mymsh->GetSolutionDof(j, iel, SolType2);
        for (unsigned idim = 0; idim < dim; idim++) {
          vx[idim][j] = vx_hat[idim][j] + Soli[indexVAR[idim + 2 * dim]][j];
          vx_old[idim][j] = vx_hat[idim][j] + Soli_old[indexVAR[idim + 2 * dim]][j];
        }
      }
          ...
          if (myel->GetFaceElementIndex(iel, jface) < 0) {
            ...
            if ((!ml_sol->GetBdcFunction()(xx, "P", tau, face, time) &&
              ...
              for (unsigned i = 0; i < nve; i++) {
                ...
                for (unsigned idim = 0; idim < dim; idim++) {
                  vx_face[idim][i]    = (*mymsh->_topology->_Sol[idim])(idof) + Soli[indexVAR[idim + 2 * dim]][ilocal]; //TODO
                  vx_face_old[idim][i] = (*mymsh->_topology->_Sol[idim])(idof) + Soli_old[indexVAR[idim + 2 * dim]][ilocal];
                }
              }
...
// 					      - IRe * LapvelVAR_old[idim]	       	         // viscous dissipation
		...
		for (int idim = 0; idim < dim; idim++) {
		  aRhs[indexVAR[idim + 2 * dim]][i] += ( - 1.0e-5 * LapAuxVAR[idim] + ( SolVAR[idim + 2 * dim] - SolVAR[idim] ) * phi[i] ) * Weight;
		}
...
//
		...
		for (int idim = 0; idim < dim; idim++) {
		  aRhs[indexVAR[idim + 2 * dim]][i] += ( - 1.0e-3 * LapAuxVAR[idim] + ( SolVAR[idim + 2 * dim] - SolVAR[idim] ) * phi[i] ) * Weight;
		}
		...
		//END auxiliary displacement equation
          ...
          //BEGIN continuity block
          {
            for (unsigned i = 0; i < nve1; i++) {
              if (!penalty) {
                if (0 == solid_model) {
                  aRhs[indexVAR[3 * dim]][i] += - (-phi1[i] * (I_e + (!incompressible) / lambda * SolVAR[2 * dim])) * Weight_hat;
                }
                else if (1 == solid_model || 5 == solid_model) {
                  aRhs[indexVAR[3 * dim]][i] += phi1[i] * (J_hat - 1. + (!incompressible) / lambda * SolVAR[2 * dim]) * Weight_hat;
                }
