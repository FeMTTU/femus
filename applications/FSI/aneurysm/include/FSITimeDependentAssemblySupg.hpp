            ...
            for (unsigned i = 0; i < nve1; i++) {
              if (!penalty) {
                if (0 == solid_model) {
                  ...
                  aRhs[indexVAR[nBlocks * dim]][i] += phi1[i] * (J_hat - 1. + (!incompressible) / lambda * SolVAR[nBlocks * dim]) * Weight_hat;
                }
...
// 		}
  ...
  void FSITimeDependentAssemblySupgNew1(MultiLevelProblem & ml_prob)
  {
    ...
    vector< vector< adept::adouble > > Soli(3 * dim + 1);
    vector< vector< double > > Soli_old(3 * dim + 1);
    vector< vector< int > > dofsVAR(3 * dim + 1);
    ...
    vector< vector< double > > Rhs(3 * dim + 1);
    vector< vector< adept::adouble > > aRhs(3 * dim + 1);
    ...
    vector < int > dofsAll;
    dofsAll.reserve(max_size * (3 * dim + 1));

    //vector < double > KKloc;
    //KKloc.reserve ( dim * max_size * ( 2 * dim + 1 ) *dim * max_size * ( 2 * dim + 1 ) );

    vector < double > Jac;
    Jac.reserve(dim * max_size * (3 * dim + 1) *dim * max_size * (3 * dim + 1));
    ...
    //variable-name handling
    const char varname[10][4] = { "DX", "DY", "DZ", "U", "V", "W", "DX1", "DY1", "DZ1", "P"};
    vector <unsigned> indexVAR(3 * dim + 1);
    ...
    indexVAR[3 * dim] = my_nnlin_impl_sys.GetSolPdeIndex(&varname[9][0]);
    indVAR[3 * dim] = ml_sol->GetIndex(&varname[9][0]);
    SolType[3 * dim] = ml_sol->GetSolutionType(&varname[9][0]);
    //----------------------------------------------------------------------------------
    ...
    for (int iel = mymsh->_elementOffset[iproc]; iel < mymsh->_elementOffset[iproc + 1]; iel++) {
      ...
      dofsVAR[3 * dim].resize(nve1);
      Soli[indexVAR[3 * dim]].resize(nve1);
      Soli_old[indexVAR[3 * dim]].resize(nve1);
      aRhs[indexVAR[3 * dim]].resize(nve1);

      dofsAll.resize(0);

      Jac.resize((3 * dim * nve + nve1) * (3 * dim * nve + nve1));
      ...
      // pressure dofs
      for (unsigned i = 0; i < nve1; i++) {
        unsigned idof = mymsh->GetSolutionDof(i, iel, SolType[3 * dim]);
        dofsVAR[3 * dim][i] = myLinEqSolver->GetSystemDof(indVAR[3 * dim], indexVAR[3 * dim], i, iel);
        Soli[indexVAR[3 * dim]][i]     = (*mysolution->_Sol[indVAR[3 * dim]])(idof);
        Soli_old[indexVAR[3 * dim]][i] = (*mysolution->_SolOld[indVAR[3 * dim]])(idof);
        aRhs[indexVAR[3 * dim]][i] = 0.;
      }
      ...
      dofsAll.insert(dofsAll.end(), dofsVAR[3 * dim].begin(), dofsVAR[3 * dim].end());
      ...
      for (int j = 0; j < nve; j++) {
        unsigned idof = mymsh->GetSolutionDof(j, iel, SolType2);
        for (unsigned idim = 0; idim < dim; idim++) {
          vx[idim][j] = vx_hat[idim][j] + Soli[indexVAR[idim + 0 * dim]][j];
          vx_old[idim][j] = vx_hat[idim][j] + Soli_old[indexVAR[idim + 0 * dim]][j];
        }
      }
          ...
          if (myel->GetFaceElementIndex(iel, jface) < 0) {
            ...
            if ((!ml_sol->GetBdcFunction()(xx, "P", tau, face, time) &&
              ...
              for (unsigned i = 0; i < nve; i++) {
                ...
                for (unsigned idim = 0; idim < dim; idim++) {
                  vx_face[idim][i]    = (*mymsh->_topology->_Sol[idim])(idof) + Soli[indexVAR[idim + 0 * dim]][ilocal]; //TODO
                  vx_face_old[idim][i] = (*mymsh->_topology->_Sol[idim])(idof) + Soli_old[indexVAR[idim + 0 * dim]][ilocal];
                }
              }
      ...
      for (unsigned ig = 0; ig < mymsh->_finiteElement[ielt][SolType2]->GetGaussPointNumber(); ig++) {
        ...
        for (unsigned inode = 0; inode < nve1; inode++) {
          SolVAR[3 * dim]    += phi1[inode] * Soli[indexVAR[3 * dim]][inode];
        }
        ...
        for (unsigned i = 0; i < dim; i++) {
          meshVel[i] = (SolVAR[i] - SolVAR_old[i]) / dt; //TODO maybe we should use DX1-DY1
          GradMeshVel[i].resize(dim);
              ...
              if (flag_mat == 2) {
                ...
                for (int idim = 0; idim < dim; idim++) {
                  ...
                  adept::adouble value =  theta * (
                                            ...
                                            - IRe * LapvelVAR[idim]	             // viscous dissipation
                                            + IRe * LapStrong[idim]
                                            + 1. / rhof * SolVAR[3 * dim] * gradphi[i * dim + idim] // pressure gradient
                                          ) * Weight;                                // at time t
                  ...
                  adept::adouble value_old = (1. - theta) * (
                                               ...
                                               - IRe * LapvelVAR_old[idim]	       	         // viscous dissipation
                                               + IRe * LapStrong_old[idim]
                                               + 1. / rhof * SolVAR[3 * dim] * gradphi_old[i * dim + idim]  // pressure gradient
                                             ) * Weight_old;			                 // at time t-dt
...
// 					      - IRe * LapvelVAR_old[idim]	       	         // viscous dissipation
                ...
                for (int idim = 0; idim < dim; idim++) {
                  for (int jdim = 0; jdim < dim; jdim++) {
                    //LapmapVAR[idim] += ( GradSolVAR[idim][jdim] ) * gradphi_hat[i * dim + jdim];
                    LapAuxVAR[idim] += (GradSolVAR[idim + 2 * dim][jdim]
                                        //+ 0.*GradSolVAR[jdim + 2 * dim][idim]
                                       ) * gradphi[i * dim + jdim];
                  }
                }

                for (int idim = 0; idim < dim; idim++) {
                  aRhs[indexVAR[idim + 2 * dim]][i] += (  LapAuxVAR[idim] + 0 * ( SolVAR[idim + 2 * dim] - SolVAR[idim] ) * phi[i] ) * Weight;
                }
          ...
          //BEGIN continuity block
          {
            ...
            for (unsigned i = 0; i < nve1; i++) {
              aRhs[indexVAR[3 * dim]][i] += - (-phi1[i] * div_vel) * Weight;
            }
          }
          //END continuity block ===========================
        }
        ...
        //BEGIN SOLID ASSEMBLY ============
          ...
          if (solid_model == 0) {   // Saint-Venant
            ...
            for (int i = 0; i < dim; i++) {
              for (int j = 0; j < dim; j++) {
                //incompressible
                Cauchy[i][j]     = 2 * mus * e[i][j] - 2 * mus * I_e * SolVAR[3 * dim] * Id2th[i][j];
                Cauchy_old[i][j] = 2 * mus * e_old[i][j] - 2 * mus * I_e_old * SolVAR[3 * dim] * Id2th[i][j];
                //+(penalty)*lambda*I_e*Id2th[i][j];
              }
            }
          }
            ...
            if (solid_model <= 4) {   // Neo-Hookean
              ...
              for (int I = 0; I < 3; ++I) {
                for (int J = 0; J < 3; ++J) {
                  if (1 == solid_model) {   //Wood-Bonet J_hat  =1;
                    Cauchy[I][J]     = mus * (B[I][J]     - Id2th[I][J]) - mus / 3.*I1_B     * SolVAR[3 * dim] * Id2th[I][J];
                    Cauchy_old[I][J] = mus * (B_old[I][J] - Id2th[I][J]) - mus / 3.*I1_B_old * SolVAR[3 * dim] * Id2th[I][J];
                  }
...
//
          ...
          //BEGIN continuity block
          {
            for (unsigned i = 0; i < nve1; i++) {
              if (!penalty) {
                if (0 == solid_model) {
                  aRhs[indexVAR[3 * dim]][i] += - (-phi1[i] * (I_e + (!incompressible) / lambda * SolVAR[3 * dim])) * Weight_hat;
                }
                else if (1 == solid_model || 5 == solid_model) {
                  aRhs[indexVAR[3 * dim]][i] += phi1[i] * (J_hat - 1. + (!incompressible) / lambda * SolVAR[3 * dim]) * Weight_hat;
                }
...
// 		}
      ...
      Rhs[indexVAR[3 * dim]].resize(nve1);

      for (unsigned j = 0; j < nve1; j++) {
        Rhs[indexVAR[3 * dim]][j] = -aRhs[indexVAR[3 * dim]][j].value();
      }
      ...
      if (assembleMatrix) {
        ...
        s.dependent(&aRhs[indexVAR[3 * dim]][0], nve1);
        s.independent(&Soli[indexVAR[3 * dim]][0], nve1);
