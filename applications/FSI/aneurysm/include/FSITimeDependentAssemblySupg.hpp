    ...
    for(int iel = mymsh->_elementOffset[iproc]; iel < mymsh->_elementOffset[iproc + 1]; iel++) {
        ...
        for(unsigned ig = 0; ig < mymsh->_finiteElement[ielt][SolType2]->GetGaussPointNumber(); ig++) {
            ...
            if(flag_mat == 2 || flag_mat == 3 ) {
                ...
                //vector < adept::adouble > phiSupg(nve);
		//vector < adept::adouble > phiSupg_old(nve);
                adept::adouble phiSupg=0.;
		adept::adouble phiSupg_old=0.;
                for(unsigned i = 0; i < nve; i++) {
                    for(unsigned j=0; j<dim; j++) {
                        phiSupg   += ( ( SolVAR[j+dim] - meshVel[j]) * gradphi[i*dim+j] ) * tauSupg;
			phiSupg_old   += ( ( SolVAR_old[j+dim]- meshVel[j] ) * gradphi_old[i*dim+j] ) * tauSupg_old;
                    }
                }
