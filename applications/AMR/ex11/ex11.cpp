...
static void show_usage()
{
  ...
  std::cout << "e.g.: ./AMR_ex11 --inputfile ./input/input.json" << std::endl;
}
...
//   double pi = acos(-1.);
//   double theta0 = ( x[0] < 0 || x[1] < 0 ) ? pi : 0;
//   value = (x[0] != 0 ) ? solExact
//           : pow(x[0] * x[0] + x[1] * x[1], 1. / 3) * sin( 2. / 3 * (pi / 2. + theta0));
	  
  double pi = acos(-1.);	  
  bool dirichlet = true; //dirichlet
  value = atan(60.*(sqrt((x[0]-5./4.)*(x[0]-5./4.)+(x[1]+1./4.)*(x[1]+1./4.)) - pi/3.));	  
...
//   if (iproc == 0) printf("\n||Sol_h-Sol||_H1 / ||Sol||_H1  = %g \n", H1error);
  if(AMRnorm == "L2" || AMRnorm == "l2" || AMRnorm == "H0" || AMRnorm == "h0") {
  double l2error = GetRelativeError(ml_sol, false);

  if (iproc == 0) printf("\n||Sol_h-Sol||_L2 / ||Sol||_L2  = %g \n", l2error);
   ml_msh.PrintInfo();
  }
  else if(AMRnorm == "H1" || AMRnorm == "h1" ) {
  ...
  for (int ilevel = gridn - 1; ilevel < gridn; ilevel++) {
    ...
    for (int iel = msh->GetElementOffset(iproc); iel < msh->GetElementOffset(iproc + 1); iel++) {
      ...
      for (unsigned ig = 0; ig < ml_sol.GetMLMesh()->_finiteElement[ielt][SolOrder]->GetGaussPointNumber(); ig++) {
        ...
        //current solution
        double SolT = 0;
        std::vector < double > gradSolT(3, 0.);
	...
	if ( test ) {
        ...
        // 	std::cout <<SolExact - SolT<<std::endl;
         	...
         	std::cout <<dSolExactdy - gradSolT[1]<<std::endl<<std::endl;;
