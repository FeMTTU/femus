...
//   mlMsh.ReadCoarseMesh("./input/square_tri.neu", "seventh", scalingFactor);
  ...
  std::vector < std::vector < double > > H1normE;
  H1normE.resize (maxNumberOfMeshes);

  std::vector < std::vector < double > > H1norm;
  ...
  for(unsigned i = 0; i < maxNumberOfMeshes; i++) {
    ...
    bool elementsHaveBeenRefined = meshcoarser.FlagElementsToBeRefined(0.0001, mlSol.GetSolutionLevel(numberOfUniformLevels-1)->GetSolutionByName("Error"));  //non-uniform
...
std::pair < double, double > GetError(MultiLevelSolution* mlSol) {
  ...
  for(int iel = msh->GetElementOffset(iproc); iel < msh->GetElementOffset(iproc + 1); iel++) {
	...
	seminorm2 += gradUig[j] * gradUig[j] * weight;
      ...
      for (unsigned j = 0; j < dim ; j++) {
        seminormE   += ( (gradUig[j] - solGrad[j]) * (gradUig[j] - solGrad[j])) * weight;
      }
  ...
  norm_vec->set (iproc, l2normE);
  norm_vec->close();

  l2normE = norm_vec->l1_norm();

  norm_vec->set (iproc, seminormE);
  norm_vec->close();

  seminormE = norm_vec->l1_norm();
