...
void AssembleConformalMinimization (MultiLevelProblem& ml_prob) {
  ...
  unsigned solLType = mlSol->GetSolutionType (solLIndex);
  ...
  unsigned solLPdeIndex = mlPdeSys->GetSolPdeIndex ("Lambda");
  ...
  for (int iel = msh->_elementOffset[iproc]; iel < msh->_elementOffset[iproc + 1]; iel++) {
    ...
    for (unsigned i = 0; i < nLDofs; i++) {
      // Global-to-local mapping between X solution node and solution dof.
      unsigned iDDof = msh->GetSolutionDof (i, iel, solLType);
      solL[i] = (*sol->_Sol[solLIndex]) (iDDof);
    ...
    for (unsigned ig = 0; ig < msh->_finiteElement[ielGeom][solType]->GetGaussPointNumber(); ig++) {
      ...
      adept::adouble N3;
      adept::adouble absN3;
      //Rnormal[0] = (solx_uv[1][0] * solx_uv[2][1] - solx_uv[2][0] * solx_uv[1][1]);
      //Rnormal[1] = (solx_uv[2][0] * solx_uv[0][1] - solx_uv[0][0] * solx_uv[2][1]);
      ...
      for (unsigned k = 0; k < dim; k++) {
        for (unsigned i = 0; i < nxDofs; i++) {
          ...
          if (k == 0) {
            aResDx[k][i] += lambda * (solx_uv[1][1] * phix_uv[0][i] - solx_uv[1][0] * phix_uv[1][i]) * (N3 - absN3) * Area2;
          }
          else {
            aResDx[k][i] += lambda * (-solx_uv[0][1] * phix_uv[0][i] + solx_uv[0][0] * phix_uv[1][i]) * (N3 - absN3) * Area2;
          }
        }
      }
      for (unsigned i = 0; i < nLDofs; i++) {
        aResL[i] += phiL[i] * ( (N3 - absN3) + 0.00001 * solL[i] ) * Area2 ;
      }
