...
void AssembleConformalMinimization (MultiLevelProblem& ml_prob) {
  ...
  double* phiP;  // local test function for velocity
  ...
  unsigned solPIndex = mlSol->GetIndex ("P");
  ...
  unsigned solPPdeIndex = mlPdeSys->GetSolPdeIndex ("P");
  ...
  for (int iel = msh->_elementOffset[iproc]; iel < msh->_elementOffset[iproc + 1]; iel++) {
    ...
    unsigned nPDofs  = msh->GetElementDofNumber (iel, solPType);
    ...
    // Resize local arrays
    SYSDOF.resize (dim * nxDofs + nPDofs);
    Res.resize (dim * nxDofs + nPDofs);

    for (unsigned k = 0; k < dim; k++) {
      aResDx[k].assign (nxDofs, 0.);
    }
    aResP.assign (nPDofs, 0.);
    ...
    for (unsigned i = 0; i < nxDofs; i++) {
      ...
      for (unsigned K = 0; K < DIM; K++) {
        solU[K][i] = (*sol->_Sol[solUIndex[K]]) (iDof);
        solDx[K][i] = (*sol->_Sol[solDxIndex[K]]) (iDof);
    ...
    for (unsigned i = 0; i < nPDofs; i++) {
      // Global-to-local mapping between X solution node and solution dof.
      unsigned iDof = msh->GetSolutionDof (i, iel, solPType);
      solP[i] = (*sol->_Sol[solPIndex]) (iDof);
      SYSDOF[ dim * nxDofs + i] = pdeSys->GetSystemDof (solPIndex, solPPdeIndex, i, iel);
    }
...
//         }
        ...
        for (unsigned k = 0; k < dim; k++) {
          ...
          aResDx[k][i] += term1 * Area2 - solPg * phi1_x[i * dim + k] * weight;

        }
      }
      
      for (unsigned i = 0; i < nPDofs; i++) {
          aResP[i] += - phiP[i] * ( (J_hat - 1.) + 0.000001 * solPg ) * Area2; 
      }
    ...
    for (int i = 0; i < nPDofs; i++) {
      Res[ dim * nxDofs + i] = -aResP[i].value();
    }

    RES->add_vector_blocked (Res, SYSDOF);

    // Resize Jacobian.
    Jac.resize ( (dim * nxDofs + nPDofs) * (dim * nxDofs + nPDofs));

    // Define the dependent variables.
    for (int k = 0; k < dim; k++) {
      s.dependent (&aResDx[k][0], nxDofs);
    }
    s.dependent (&aResP[0], nPDofs);

    // Define the independent variables.
    for (int k = 0; k < dim; k++) {
      s.independent (&solDx[k][0], nxDofs);
    }
    s.independent (&solP[0], nPDofs);
