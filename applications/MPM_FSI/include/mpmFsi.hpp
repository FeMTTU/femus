...
void AssembleMPMSys (MultiLevelProblem& ml_prob) {
  ...
  unsigned indexSolP = ml_sol->GetIndex ("P");
  unsigned indexPdeP = my_nnlin_impl_sys.GetSolPdeIndex ("P");
  unsigned solTypeP = ml_sol->GetSolutionType ("P");
  ...
  for (int iel = mymsh->_elementOffset[iproc]; iel < mymsh->_elementOffset[iproc + 1]; iel++) {
    ...
    for (unsigned i = 0; i < nDofsP; i++) {
      unsigned idof = mymsh->GetSolutionDof (i, iel, solTypeP);
    ...
    for (unsigned ig = 0; ig < mymsh->_finiteElement[ielt][solType]->GetGaussPointNumber(); ig++) {
      ...
      double *phiP = mymsh->_finiteElement[ielt][solTypeP]->GetPhi (ig);
    ...
    if (iel != UINT_MAX) {
      ...
      if (iel != ielOld) {
        ...
        for (unsigned k = 0; k < dim; k++) {
          ...
          aRhsD[k].assign (nDofsD, 0.);
          vx[k].resize (nDofsD);
          vxHat[k].resize (nDofsD);
        }
        sysDofsAll.resize (dim * nDofsD);
        ...
        for (unsigned i = 0; i < nDofsD; i++) {
          ...
          for (unsigned k = 0; k < dim; k++) {
            ...
            sysDofsAll[k * nDofsD + i ] = myLinEqSolver->GetSystemDof (indexSolD[k], indexPdeD[k], i, iel);
...
void AssembleMPMSysOldOld (MultiLevelProblem& ml_prob) {
