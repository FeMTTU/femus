...
void AssembleMPMSys (MultiLevelProblem& ml_prob) {
  ...
  for (int iel = msh->_elementOffset[iproc]; iel < msh->_elementOffset[iproc + 1]; iel++) {
    ...
    bool test = (counter >= nDofs - 1) ? true : false;
    ...
    for (unsigned ig = 0; ig < msh->_finiteElement[ielt][solType]->GetGaussPointNumber(); ig++) {
      ...
      for (unsigned i = 0; i < nDofsP; i++) {
        for (unsigned  k = 0; k < dim; k++) {
          if (MPMmaterial == nDofs || test) {  //all cells that are completely MPM solid
...
//       particles[iMarker]->SetMarkerAcceleration (particleAcc);
      ...
      particles[iMarker]->GetMarkerTangent (tangentHat);

      std::vector <std::vector < double > > tangent;
      std::vector < double > ds;

      tangent.resize (tangentHat.size());
      ds.assign (tangentHat.size(), 0.);

      for (unsigned k = 0; k < tangent.size(); k++) {
        tangent[k].assign (dim, 0.);
        for (unsigned i = 0; i < dim; i++) {
          for (unsigned j = 0; j < dim; j++) {
            tangent[k][i] += Fp[i][j] * tangentHat[k][j];
          }
        }
        for (unsigned i = 0; i < dim; i++) {
          ds[k] += tangent[k][i] * tangent[k][i];
        }
        ds[k] = sqrt (ds[k]);
      }
      ...
      if (dim == 2) {
        normal[0] =  tangent[0][1] / dA;
        normal[1] = -tangent[0][0] / dA;
...
void AssembleMPMSysOld (MultiLevelProblem& ml_prob) {
  ...
  std::vector < unsigned > sysDofsD;
  std::vector < unsigned > sysDofsV;
  ...
  for (unsigned iMarker = markerOffsetSolid[iproc]; iMarker < markerOffsetSolid[iproc + 1]; iMarker++) {
    ...
    if (iel != UINT_MAX) {
      ...
      if (iel != ielOld) { //update element related quantities only if we are in a different element
        ...
        sysDofsD.resize (dim * nDofs);
        sysDofsV.resize (dim * nDofs);
        ...
        for (unsigned i = 0; i < nDofs; i++) {
          ...
          for (unsigned k = 0; k < dim; k++) {
            ...
            sysDofsD[k * nDofs + i ] = myLinEqSolver->GetSystemDof (indexSolD[k], indexPdeD[k], i, iel);
            sysDofsV[k * nDofs + i ] = myLinEqSolver->GetSystemDof (indexSolV[k], indexPdeV[k], i, iel);
      ...
      if (iMarker == markerOffsetSolid[iproc + 1] - 1 || iel != particlesSolid[iMarker + 1]->GetMarkerElement()) {
        ...
        myRES->add_vector_blocked (rhs, sysDofsD);
        ...
        myKK->add_matrix_blocked (Jac, sysDofsD, sysDofsD);
        s.clear_independents();
        s.clear_dependents();

        if (MPMmaterial < nDofs) { // add this contribution only if this is an inteface element
          rhs.resize (nDofs * dim);
          for (unsigned k = 0; k < dim; k++) {
            for (unsigned i = 0; i < nDofs; i++) {
          ...
          myRES->add_vector_blocked (rhs, sysDofsV);
          ...
          myKK->add_matrix_blocked (Jac, sysDofsV, sysDofsD);
          s.clear_independents();
          s.clear_dependents();
        }
      }
  ...
  //   std::vector<Marker*> particlesInterface = solidLine->GetParticles();
  //
  //   ielOld = UINT_MAX;
  //   std::vector < unsigned > sysDofsDV;
  //
  //   for (unsigned iMarker = markerOffsetInterface[iproc]; iMarker < markerOffsetInterface[iproc+1]; iMarker++) {
  //     unsigned iel = particlesInterface[iMarker]->GetMarkerElement();
  //
  //     short unsigned ielt;
  //     unsigned nDofs;
  //     unsigned nDofsP;
  ...
  //
  //     if (iel != UINT_MAX) {
  //       unsigned material = (*mysolution->_Sol[indexSolMat]) (iel);
  //       if ( material > 0 && material < nDofs ) {
  //
  //         //update element related quantities only if we are in a different element
  //         if (iel != ielOld) {
  //
  //           ielt = msh->GetElementType (iel);
  //           nDofs = msh->GetElementDofNumber (iel, solType);   // number of solution element dofs
  //           nDofsP = msh->GetElementDofNumber (iel, solTypeP);
  //           nDofsAll = 2 * dim * nDofs + nDofsP;
  //
  //           sysDofsAll.resize (nDofsAll);
  //
  //           solidFlag.resize (nDofs);
  ...
  //             }
  //
  //             test = (counter >= nDofs - 1) ? true : false;
  //           }
  //
  //           for (unsigned i = 0; i < nDofsP; i++) {
  //             unsigned idof = msh->GetSolutionDof (i, iel, solTypeP);
  ...
  //         double mass = particlesInterface[iMarker]->GetMarkerMass();
  //
  //         msh->_finiteElement[ielt][solType]->Jacobian (vx, xi, weight, phi, gradPhi);
  //
  //
  //         //std::vector <double> SolVpOld (dim);
  //         //     particlesInterface[iMarker]->GetMarkerVelocity (SolVpOld);//TODO
  //
  //         //BEGIN evaluates SolDp at the particle iMarker
  //         vector<adept::adouble> solDp (dim, 0.);
  //         vector<double> solDpOld (dim, 0.);
  ...
  //         }
  //
  //         if (iMarker == markerOffset2 - 1 || iel != particlesInterface[iMarker + 1]->GetMarkerElement()) {
  //
  //           rhs.resize (nDofsAll); //resize
  //
  //           for (int i = 0; i < nDofs; i++) {
  ...
  for (unsigned iMarker = markerOffsetFluid[iproc]; iMarker < markerOffsetFluid[iproc + 1]; iMarker++) {
    //element of particle iMarker
    unsigned iel = particlesFluid[iMarker]->GetMarkerElement();
    ...
    if (iel != UINT_MAX) {
      ...
      if ( (*mysolution->_Sol[indexSolMat]) (iel) > 0) {
        ...
        if (iMarker == markerOffsetFluid[iproc + 1] - 1 || iel != particlesFluid[iMarker + 1]->GetMarkerElement()) {
