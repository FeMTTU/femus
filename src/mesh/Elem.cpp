...
namespace femus {
  ...
  void elem::ScatterElementCanBeRefinedVector() {
    ...
    for (unsigned iel = _elementOffset; iel < _elementOffsetP1; iel++) {
      _elementCanBeRefined[iel - _elementOffset] = tempElementCanBeRefined[iel];
    }
  ...
  bool elem::GetIfElementCanBeRefined(const unsigned& iel) const {
    return (_elementCanBeRefinedIsScattered == false) ?
           _elementCanBeRefined[iel] : _elementCanBeRefined[iel - _elementOffset];
  }
  ...
  **/
  ...
  void elem::ScatterElementDof() {
    ...
    if (_localElementDofMemorySize == 0) {
      _localElementDofMemorySize = _elementDof[_elementOffsetP1] - _elementDof[_elementOffset];

      _elementDofOffset = _elementDof[_elementOffset] - _elementDof[0];

      _localElementDofMemory = new unsigned [_localElementDofMemorySize];

      for (unsigned i = 0; i < _localElementDofMemorySize; i++) {
        _localElementDofMemory[i] =  *(_elementDof[_elementOffset] + i);
      }
      ...
      for (unsigned i = 0; i < _elementOwned; i++) {
        ...
        pt_u += _elementDof[_elementOffset + (i + 1)] - _elementDof[_elementOffset + i];
      }
  ...
  void elem::LocalizeElementDofToOne(const unsigned& isdom) {

    unsigned* sendBuffer = new unsigned [_elementDofMemorySize];
    memset(sendBuffer, 0, _elementDofMemorySize * sizeof(unsigned));
    for (unsigned i = 0; i < _localElementDofMemorySize; i++) {
      sendBuffer [_elementDofOffset + i] = _localElementDofMemory[i];
    }
    if(isdom == _iproc)_elementDofMemory = new unsigned [_elementDofMemorySize];
    MPI_Reduce(sendBuffer, _elementDofMemory, _elementDofMemorySize, MPI_UNSIGNED, MPI_SUM, isdom, MPI_COMM_WORLD);
    delete [] sendBuffer;

    sendBuffer = new unsigned [_nel];
    memset(sendBuffer, 0, _nel * sizeof(unsigned));
    for (unsigned i = 0; i < _elementOwned; i++) {
      sendBuffer[_elementOffset + i] = _localElementDof[i + 1] - _localElementDof[i];
    }

    unsigned* elSize;
    if(isdom == _iproc) elSize = new unsigned [_nel];
    MPI_Reduce(sendBuffer, elSize, _nel, MPI_UNSIGNED, MPI_SUM, isdom, MPI_COMM_WORLD);
    delete [] sendBuffer;
  ...
  void elem::LocalizeElementDofToAll() {
    ...
    _elementDofMemory = new unsigned [_elementDofMemorySize];
    MPI_Allreduce(sendBuffer, _elementDofMemory, _elementDofMemorySize, MPI_UNSIGNED, MPI_SUM, MPI_COMM_WORLD);
    delete [] sendBuffer;
    ...
    unsigned* elSize = new unsigned [_nel];
