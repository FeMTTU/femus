...
//------------------------------------- Initialization for k = 0,1,2,3,4 -------------------------------------
  ...
  IS_Gmt2Mts_dof_counter[3]=0; // probably to remove
  IS_Gmt2Mts_dof_counter[4]=0; // probably to remove
...
//--------------------------------------------- k = 3,4 ---------------------------------------------
  ...
  // ghost vs own nodes is probably to remove TODO
...
for(unsigned k=0; k<3; k++) {
  ...
  IS_Mts2Gmt_elem_offset[0] = 0;  // not sure about this! TODO
  ...
  // ghost vs own nodes
  vector <unsigned short> node_count(_nnodes,0); //should it be declared outside the for loop on k?
  ...
  for(int isdom=0;isdom<nsubdom;isdom++) {
	...
	if(node_count[ii]<isdom+1){
	  ...
	  if(npart[ii] != isdom){
	    for(ik=0; ik<=k; ik++){
 	    ghost_nd_mts[ik][isdom][ghost_size[ik][isdom]]=IS_Gmt2Mts_dof[ik][ii];
            ghost_nd[ik][isdom][ghost_size[ik][isdom]]=ii;
	    ghost_size[ik][isdom]++;
	    }
	  }
	}
...
//--------------------------------------------------- Final part that holds for every k = 0,1,2,3,4 ---------------------------------
