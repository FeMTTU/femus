...
void MultiLevelMesh::ReadCoarseMesh(const char mesh_file[], const char GaussOrder[], const double Lref)
{
  ...
  const bool read_groups = true; //by default groups are read
    ...
    ReadCoarseMesh(mesh_file, GaussOrder, Lref, read_groups);
   
}


void MultiLevelMesh::ReadCoarseMesh(const char mesh_file[], const char GaussOrder[], const double Lref, const bool read_groups)
{
    _gridn0 = 1;
    ...
    _level0[0] = new Mesh();
    std::cout << " Reading corse mesh from file: " << mesh_file << std::endl;
    _level0[0]->ReadCoarseMesh(mesh_file, Lref,_finiteElementGeometryFlag, read_groups);
