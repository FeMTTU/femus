...
void QuantityLocal::VectWithQtyFillBasic() {
  ...
  if ( _qtyptr == NULL ) {std::cout << " Vect must be endowed with a quantity for this to work " << std::endl; abort();}
    ...
    _ndof[VV] = _currEl._eqnmap._AbstractFE[VV][_FEord]->_ndof[VV];  //TODO here we use the eqnmap, of Course Vect must be used where the eqmap is there, but that can be in the main also
    _ndof[BB] = _currEl._eqnmap._AbstractFE[VV][_FEord]->_ndof[BB];
