...
{ 
  ...
  for (unsigned ig=_gridr-1u; ig<_gridn-1u; ig++) {
    nel+=( _ml_probl._msh[ig]->GetElementNumber() - _ml_probl._msh[ig]->el->GetRefinedElementNumber());
    counter+=(_ml_probl._msh[ig]->el->GetElementNumber("Hex")-_ml_probl._msh[ig]->el->GetRefinedElementNumber("Hex"))*NVE[0][index];
    counter+=(_ml_probl._msh[ig]->el->GetElementNumber("Tet")-_ml_probl._msh[ig]->el->GetRefinedElementNumber("Tet"))*NVE[1][index];
    counter+=(_ml_probl._msh[ig]->el->GetElementNumber("Wedge")-_ml_probl._msh[ig]->el->GetRefinedElementNumber("Wedge"))*NVE[2][index];
    counter+=(_ml_probl._msh[ig]->el->GetElementNumber("Quad")-_ml_probl._msh[ig]->el->GetRefinedElementNumber("Quad"))*NVE[3][index];
    counter+=(_ml_probl._msh[ig]->el->GetElementNumber("Triangle")-_ml_probl._msh[ig]->el->GetRefinedElementNumber("Triangle"))*NVE[4][index];
    counter+=(_ml_probl._msh[ig]->el->GetElementNumber("Line")-_ml_probl._msh[ig]->el->GetRefinedElementNumber("Line"))*NVE[5][index];
  }
  nel+=_ml_probl._msh[_gridn-1u]->GetElementNumber();
  counter+=_ml_probl._msh[_gridn-1u]->el->GetElementNumber("Hex")*NVE[0][index];
  counter+=_ml_probl._msh[_gridn-1u]->el->GetElementNumber("Tet")*NVE[1][index];
  counter+=_ml_probl._msh[_gridn-1u]->el->GetElementNumber("Wedge")*NVE[2][index];
  counter+=_ml_probl._msh[_gridn-1u]->el->GetElementNumber("Quad")*NVE[3][index];
  counter+=_ml_probl._msh[_gridn-1u]->el->GetElementNumber("Triangle")*NVE[4][index];
  counter+=_ml_probl._msh[_gridn-1u]->el->GetElementNumber("Line")*NVE[5][index]; 
  ...
  for(unsigned ig=_gridr-1u; ig<_gridn; ig++) {
    ...
    mysol[ig]->init(_ml_probl._msh[ig]->MetisOffset[index_nd][_nprocs],_ml_probl._msh[ig]->own_size[index_nd][_iproc],true,AUTOMATIC);
  }
  ...
  if (_moving_mesh) {
    ...
    if(_ml_probl._msh[0]->GetDimension() == 3) {
    ...
    for(unsigned ig=_gridr-1u; ig<_gridn; ig++){
      std::vector<double> v_local;
      unsigned nvt_ig=_ml_probl._msh[ig]->MetisOffset[index_nd][_nprocs];
      for(int kk=0;kk<_ml_probl._msh[0]->GetDimension();kk++) {
  ...
  for (unsigned ig=_gridr-1u; ig<_gridn; ig++) {
    for (unsigned iel=0; iel<_ml_probl._msh[ig]->GetElementNumber(); iel++) {
      if (_ml_probl._msh[ig]->el->GetRefinedElementIndex(iel)==0 || ig==_gridn-1u) {
        for (unsigned j=0; j<_ml_probl._msh[ig]->el->GetElementDofNumber(iel,index); j++) {
	  unsigned jnode=_ml_probl._msh[ig]->el->GetElementVertexIndex(iel,j)-1u;
	  unsigned jnode_Metis = _ml_probl._msh[ig]->GetMetisDof(jnode,index_nd);
	  var_conn[icount] = offset_nvt+jnode_Metis;
	  icount++;
	}
      }
    }
    offset_nvt+=_ml_probl._msh[ig]->MetisOffset[index_nd][_nprocs];
  }
  ...
  for (unsigned ig=_gridr-1u; ig<_gridn; ig++) {
    for (unsigned iel=0; iel<_ml_probl._msh[ig]->GetElementNumber(); iel++) {
      if (_ml_probl._msh[ig]->el->GetRefinedElementIndex(iel)==0 || ig==_gridn-1u) {
	unsigned iel_Metis = _ml_probl._msh[ig]->GetMetisDof(iel,3);
        offset_el += _ml_probl._msh[ig]->el->GetElementDofNumber(iel_Metis,index);
  ...
  icount=0;
  for (unsigned ig=_gridr-1u; ig<_gridn; ig++) {
    for (unsigned ii=0; ii<_ml_probl._msh[ig]->GetElementNumber(); ii++) {
      if (_ml_probl._msh[ig]->el->GetRefinedElementIndex(ii)==0 || ig==_gridn-1u) {
	unsigned iel_Metis = _ml_probl._msh[ig]->GetMetisDof(ii,3);
        short unsigned ielt= _ml_probl._msh[ig]->el->GetElementType(iel_Metis);
  ...
  for (unsigned ig=_gridr-1u; ig<_gridn; ig++) {
    for (unsigned ii=0; ii<_ml_probl._msh[ig]->GetElementNumber(); ii++) {
      if ( ig==_gridn-1u || 0==_ml_probl._msh[ig]->el->GetRefinedElementIndex(ii)) {
	unsigned iel_Metis = _ml_probl._msh[ig]->GetMetisDof(ii,3);
	var_reg[icount]= _ml_probl._msh[ig]->el->GetElementGroup(ii);
	icount++;
      }
    }
  }
  ...
  for (unsigned ig=_gridr-1u; ig<_gridn; ig++) {
    for (unsigned ii=0; ii<_ml_probl._msh[ig]->GetElementNumber(); ii++) {
      if ( ig==_gridn-1u || 0==_ml_probl._msh[ig]->el->GetRefinedElementIndex(ii)) {
	unsigned iel_Metis = _ml_probl._msh[ig]->GetMetisDof(ii,3);
	var_proc[icount]=(unsigned short)(_ml_probl._msh[ig]->epart[ii]);
	icount++;
      }
    }
  }
  ...
  for (unsigned i=0; i<(1-test_all)*vars.size()+test_all*_ml_probl.SolName.size(); i++) {
	...
	for (unsigned ii=0; ii<_ml_probl._msh[ig]->GetElementNumber(); ii++) {
	  if (ig==_gridn-1u || 0==_ml_probl._msh[ig]->el->GetRefinedElementIndex(ii)) {
	    unsigned iel_Metis = _ml_probl._msh[ig]->GetMetisDof(ii,_ml_probl.SolType[indx]);
