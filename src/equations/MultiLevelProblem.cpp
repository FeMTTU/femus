...
void MultiLevelProblem::Initialize(const char name[], initfunc func) {
  ...
  for (unsigned i=i_start; i<i_end; i++) {
    ...
    for (unsigned ig=0; ig<_gridn; ig++) {
	  ...
	  unsigned sol_ord = _ml_msh->GetLevel(ig)->GetEndIndex(SolType[i]);
...
void MultiLevelProblem::BuildProlongatorMatrix(unsigned gridf, unsigned SolIndex) {
  ...
  if(_solution[gridf]->_ProjMatFlag[TypeIndex]==0){
    ...
    int nf     = _ml_msh->GetLevel(gridf)->MetisOffset[SolType[SolIndex]][_nprocs];
    int nc     = _ml_msh->GetLevel(gridf-1)->MetisOffset[SolType[SolIndex]][_nprocs];
    int nf_loc = _ml_msh->GetLevel(gridf)->own_size[SolType[SolIndex]][_iproc];
    int nc_loc = _ml_msh->GetLevel(gridf-1)->own_size[SolType[SolIndex]][_iproc]; 
...
void MultiLevelProblem::GenerateBdc(const unsigned int k, const double time) {
      ...
      for (unsigned igridn=0; igridn<_gridn; igridn++) {
	if(_solution[igridn]->_ResEpsBdcFlag[k]){
	  for (unsigned j=_ml_msh->GetLevel(igridn)->MetisOffset[SolType[k]][_iproc]; j<_ml_msh->GetLevel(igridn)->MetisOffset[SolType[k]][_iproc+1]; j++) {
	    _solution[igridn]->_Bdc[k]->set(j,2.);
	  }
	  if (SolType[k]<3) {  
	    for(int isdom=_iproc; isdom<_iproc+1; isdom++) {   // 1 DD Dirichlet
		...
		for (unsigned jface=0; jface<_ml_msh->GetLevel(igridn)->el->GetElementFaceNumber(kel_gmt); jface++) {
		  if (_ml_msh->GetLevel(igridn)->el->GetFaceElementIndex(kel_gmt,jface)==0) { //Domain Decomposition Dirichlet
		    ...
		    unsigned nv1=(!_TestIfPressure[k])?
		      ...
		      _ml_msh->GetLevel(igridn)->el->GetElementDofNumber(iel,_ml_msh->GetLevel(igridn)->GetEndIndex(SolType[k]));
		    for (unsigned iv=0; iv<nv1; iv++) {
		      unsigned inode=(!_TestIfPressure[k])? 
			...
			_ml_msh->GetLevel(igridn)->el->GetElementVertexIndex(kel_gmt,iv)-1u;
		      unsigned inode_Metis=_ml_msh->GetLevel(igridn)->GetMetisDof(inode,SolType[k]);
		      _solution[igridn]->_Bdc[k]->set(inode_Metis,1.);
		    }
		  }
		}
		...
		for (unsigned jface=0; jface<_ml_msh->GetLevel(igridn)->el->GetElementFaceNumber(kel_gmt); jface++) {
		  if (_ml_msh->GetLevel(igridn)->el->GetFaceElementIndex(kel_gmt,jface)<0) { //Dirichlet
		    ...
		    for (unsigned iv=0; iv<nv1; iv++) {
		      ...
		      bool test=_SetBoundaryConditionFunction(xx,yy,zz,SolName[k],value,-(_ml_msh->GetLevel(igridn)->el->GetFaceElementIndex(kel_gmt,jface)+1),time);
		      if (test) {
			unsigned inode_Metis=_ml_msh->GetLevel(igridn)->GetMetisDof(inode,SolType[k]);
			_solution[igridn]->_Bdc[k]->set(inode_Metis,0.);
			_solution[igridn]->_Sol[k]->set(inode_Metis,value);
		...
		for (unsigned jface=0; jface<_ml_msh->GetLevel(igridn)->el->GetElementFaceNumber(kel_gmt); jface++) {
		  if (_ml_msh->GetLevel(igridn)->el->GetFaceElementIndex(kel_gmt,jface)==0) { //Domain Decomposition Dirichlet
		    short unsigned ielt=_ml_msh->GetLevel(igridn)->el->GetElementType(kel_gmt);
		    unsigned nv1=_ml_msh->GetLevel(igridn)->el->GetElementDofNumber(kel_gmt,_ml_msh->GetLevel(igridn)->GetEndIndex(SolType[k]));
		    for (unsigned iv=0; iv<nv1; iv++) {
		      unsigned inode=(kel_gmt+iv*nel);
		      unsigned inode_Metis=_ml_msh->GetLevel(igridn)->GetMetisDof(inode,SolType[k]);
		      _solution[igridn]->_Bdc[k]->set(inode_Metis,1.);
