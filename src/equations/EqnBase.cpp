...
void EqnBase::PrintOneVarMatrixHDF5(std::string name, std::string groupname, uint** n_dofs_lev_fe,int count,
...
//
void EqnBase::PrintOneVarMGOperatorHDF5(std::string filename, std::string groupname, uint* n_dofs_lev, int count, int* Op_pos,double* Op_val,int* len,int* len_off, int FELevel_row, int FELevel_col, int fe) const {
...
//These FElevels return EXTENDED levels, to be used in the _Qnode_fine_Qnode_lev map
        ...
        n_dofs_fe_lev[QQ] = _mesh._NoNodesXLev;            //equal the pointers
        n_dofs_fe_lev[LL] = _mesh._NoNodesXLev;            //equal the pointers
        n_dofs_fe_lev[KK] = _mesh._n_elements_vb_lev[VV];  //equal the pointers
      ...
      int countoff[QL];
          ...
          for (int fe=0;fe<QL;fe++) {
	      int dim_large = n_dofs_fe_lev[fe][FEXLevel_f[fe]]*_mesh._elnodes[VV][fe];  //TODO the length of this array depends on HOW you EXPLORE things:
        ...
        for (int pr=0; pr<_mesh._NoSubdom; pr++) {
            for (int iel = _mesh._off_el[VV][pr*_NoLevels + Lev_f]; iel< _mesh._off_el[VV][pr*_NoLevels + Lev_f+1]; iel++) {
                int el_lib = _mesh._el_fm_libm[iel].second;
                int par_el = _mesh._el_sto[el_lib]->_par;
                int ch=0;
                int ch_id=0;
                while (el_lib != _mesh._el_sto[par_el]->_elchs[ch]) ch_id=++ch;

        for (int fe=0;fe<QL;fe++) {
		if (fe < KK) {
		for (uint node_of_child_elem=0; node_of_child_elem < _mesh._elnodes[VV][fe]; node_of_child_elem++) {
                    int el_n = _mesh._nd_libm_fm[ _mesh._el_sto[el_lib]->_elnds[node_of_child_elem] ];
                    for (uint node_of_par_elem=0;node_of_par_elem < _mesh._elnodes[VV][fe]; node_of_par_elem++) {
                        int par_n = _mesh._nd_libm_fm[ _mesh._el_sto[par_el]->_elnds[node_of_par_elem] ];
			  int mypp_node = _mesh._Qnode_fine_Qnode_lev[ FEXLevel_f[fe] ][el_n]*_mesh._elnodes[VV][fe]+node_of_par_elem;
                        Prol_pos[fe][ mypp_node ] = _mesh._Qnode_fine_Qnode_lev[ FEXLevel_c[fe] ][par_n];
                        Prol_val[fe][ mypp_node ] = _AbstractFE[fe]->get_embedding_matrix(ch_id, node_of_child_elem, node_of_par_elem);
                    }
                }
	     }
		else if (fe == KK)  {
		  int sum_previous_sd=0;      //sum of the dofs AT THAT LEVEL coming from the PREVIOUSLY CONSIDERED SUBDOMAINS
                   for (int sd=0;sd<pr;sd++) sum_previous_sd += _mesh._off_el[VV][ sd*_NoLevels + Lev_f+1 ] - _mesh._off_el[VV][ sd*_NoLevels+Lev_f ];
                        int  dof_r = iel - _mesh._off_el[VV][pr*_NoLevels+Lev_f] + sum_previous_sd;

		   //what is the number in femus indexing of the parent element? par_el is in libmesh indexing!
		  int iel_femus_c = _mesh._el_libm_fm[par_el];
		  int sum_previous_sd_coarse = 0;
                   for (int sd=0;sd<pr;sd++) sum_previous_sd_coarse += _mesh._off_el[VV][ sd*_NoLevels + Lev_c+1 ] - _mesh._off_el[VV][ sd*_NoLevels+Lev_c ];
                        int  dof_c = iel_femus_c - _mesh._off_el[VV][pr*_NoLevels+Lev_c] + sum_previous_sd_coarse;
        ...
        for (int fe=0; fe < QL; fe++) {
            dof_extreme_at_coarse_lev_sd[fe] = new int[_mesh._NoSubdom+1];
            dof_extreme_at_coarse_lev_sd[fe][0] = 0;
            for (int idom=0;idom<_mesh._NoSubdom;idom++){
	           if (fe  < KK)  dof_extreme_at_coarse_lev_sd[fe][idom+1] =  dof_extreme_at_coarse_lev_sd[fe][idom] + ( _mesh._off_nd[fe][idom*_NoLevels + Lev_c+1] - _mesh._off_nd[fe][idom*_NoLevels]); 
              else if (fe == KK)  dof_extreme_at_coarse_lev_sd[fe][idom+1] =  dof_extreme_at_coarse_lev_sd[fe][idom] + ( _mesh._off_el[VV][idom*_NoLevels + Lev_c+1] - _mesh._off_el[VV][idom*_NoLevels + Lev_c]);
             }
        }        
        ...
        int ki=0;
        for (int kdom=0;kdom <_mesh._NoSubdom; kdom++) {
                    int delta_dof_at_fine_lev_sd;
                    if       (fe < KK)  delta_dof_at_fine_lev_sd = _mesh._off_nd[fe][kdom*_NoLevels + Lev_f+1] - _mesh._off_nd[fe][kdom*_NoLevels];
                    else if (fe == KK)  delta_dof_at_fine_lev_sd = _mesh._off_el[VV][kdom*_NoLevels + Lev_f+1] - _mesh._off_el[VV][kdom*_NoLevels+Lev_f];
  
            for (int iki = 0; iki < delta_dof_at_fine_lev_sd; iki++) {

                for (uint kj=0;kj < _mesh._elnodes[VV][fe]; kj++) {
                    int kk= _mesh._elnodes[VV][fe]*ki+kj;
...
#ifdef DEFAULT_PRINT_INFO
    std::cout<< " EqnBase::compute_and_print_MGOps: compute_prol end  \n";
#endif
...
void EqnBase::ComputeMatrix() {

#ifdef DEFAULT_PRINT_INFO
    std::cout << " EqnBase::compute_matrix:  start \n";
#endif
...
//==============================================================
    ...
    off_dof[QQ] = _mesh._off_nd[QQ];  //You set the pointers to be equal
    off_dof[LL] = _mesh._off_nd[LL];  //You set the pointers to be equal
    off_dof[KK] = _mesh._off_el[VV];  //You set the pointers to be equal
...
//============== SET LEVELS and DOFS =======================
        ...
        n_dofs_lev_fe[QQ] = _mesh._NoNodesXLev;            //equal the pointers
        n_dofs_lev_fe[LL] = _mesh._NoNodesXLev;            //equal the pointers
        n_dofs_lev_fe[KK] = _mesh._n_elements_vb_lev[VV];  //equal the pointers
        ...
        for (int r=0; r<QL;r++) {
            ...
            for (int c=0; c<QL;c++) {
                ...
                MatG[r][c]       = new int[n_dofs_lev_fe[r][FELevel[r]]*_mesh._elnodes[VV][c]*_mesh._maxelxnode];       // or even better (n_dofs_lev_fe[level][FE_ROW]) * ( _elnodes[VV][FE_COL]*_elxnode[ROW_DOF])
                for (uint im = 0; im <     n_dofs_lev_fe[r][FELevel[r]]*_mesh._elnodes[VV][c]*_mesh._maxelxnode; im++)  MatG[r][c][im] = NegativeOneFlag;
            }
        }
...
// ========== DOF EXTREMES AT LEVEL ====================================
        ...
        for (int fe=0; fe<QL; fe++)   dof_extreme_at_lev_sd[fe] = new int [_mesh._NoSubdom+1];

        for (int fe=0; fe<QL; fe++)  {
            dof_extreme_at_lev_sd[fe][0]=0;
            for (int idom=0;idom<_mesh._NoSubdom;idom++) {
                if (fe < KK)       dof_extreme_at_lev_sd[fe][idom+1] =  dof_extreme_at_lev_sd[fe][idom] + ( _mesh._off_nd[fe][idom*_NoLevels+Level1+1] - _mesh._off_nd[fe][idom*_NoLevels]);
                else if (fe == KK) dof_extreme_at_lev_sd[fe][idom+1] =  dof_extreme_at_lev_sd[fe][idom] + ( _mesh._off_el[VV][idom*_NoLevels+Level1+1] - _mesh._off_el[VV][idom*_NoLevels+Level1]);
...
// ==============================================
        ...
        for (int pr=0;pr<_mesh._NoSubdom;pr++) {
            for (int iel = _mesh._off_el[VV][pr*_NoLevels+Level1]; iel < _mesh._off_el[VV][pr*_NoLevels+Level1+1]; iel++) {
                int el_lib = _mesh._el_fm_libm[iel].second;

                for (int r=0; r<QL;r++) {
                    for (uint k_r=0; k_r < _mesh._elnodes[VV][r]; k_r++) { //this loop can be optimized because some operations are not done for KK

                        int nd_r  = _mesh._nd_libm_fm[ _mesh._el_sto[el_lib]->_elnds[k_r] ]; //node corresponding to the row k_r
                        int dof_r = _mesh._Qnode_fine_Qnode_lev[FELevel[r]][nd_r];              //dof corresponding to the node for that level

                        if (r==KK) {
                            int sum_previous_sd=0;      //sum of the dofs AT THAT LEVEL coming from the PREVIOUSLY CONSIDERED SUBDOMAINS
                            for (int sd=0;sd<pr;sd++) sum_previous_sd += _mesh._off_el[VV][sd*_NoLevels+Level1+1] - _mesh._off_el[VV][sd*_NoLevels+Level1];
                            dof_r = iel - _mesh._off_el[VV][pr*_NoLevels+Level1] + sum_previous_sd;
                        }
// put debug control here on dof_r
                        for (int c=0; c<QL;c++) {
                            for (uint k_c=0;k_c < _mesh._elnodes[VV][c]; k_c++) {
                                int nd_c  = _mesh._nd_libm_fm[ _mesh._el_sto[el_lib]->_elnds[k_c]];  //node corresponding to the column k_c
                                int dof_c = _mesh._Qnode_fine_Qnode_lev[FELevel[c]][nd_c];

                                if (c==KK) {
                                    int sum_previous_sd=0;      //sum of the dofs AT THAT LEVEL coming from the PREVIOUSLY CONSIDERED SUBDOMAINS
                                    for (int sd=0;sd<pr;sd++) sum_previous_sd += _mesh._off_el[VV][sd*_NoLevels+Level1+1] - _mesh._off_el[VV][sd*_NoLevels+Level1];
                                    dof_c = iel - _mesh._off_el[VV][pr*_NoLevels+Level1] + sum_previous_sd;
                                }

                                MatG[r][c][ dof_r*_mesh._elnodes[VV][c]*_mesh._maxelxnode + memG[r][c][dof_r]*_mesh._elnodes[VV][c] + k_c ] = dof_c;
...
//=============================================
                ...
                aux = new int[ n_dofs_lev_fe[r][FELevel[r]]*_mesh._elnodes[VV][c]*_mesh._maxelxnode];  //This has the same length as MatG

                for (uint i=0; i < n_dofs_lev_fe[r][FELevel[r]]*_mesh._elnodes[VV][c]*_mesh._maxelxnode; i++) aux[i] = 0;
                ...
                for (int kdom=0;kdom <_mesh._NoSubdom; kdom++) {
                    ...
                    if       (r < KK)  delta_dof_at_lev_sd =  _mesh._off_nd[r][kdom*_NoLevels+Level1+1] -  _mesh._off_nd[r][kdom*_NoLevels];
                    else if (r == KK)  delta_dof_at_lev_sd = _mesh._off_el[VV][kdom*_NoLevels+Level1+1] - _mesh._off_el[VV][kdom*_NoLevels+Level1];
                    for (int iki=0; iki < delta_dof_at_lev_sd; iki++) {
                        ...
                        for (uint kj=0;kj < _mesh._elnodes[VV][c]*_mesh._maxelxnode; kj++) {
                            int kdofG = MatG[r][c][_mesh._elnodes[VV][c]*_mesh._maxelxnode*ki+kj];
...
#ifdef DEFAULT_PRINT_INFO
    std::cout << " EqnBase::compute_and_print_MGOps: compute_matrix end \n";
#endif
...
void EqnBase::ComputeRest( ) {
  ...
  double   PseudoZero = 1.e-8;
    ...
    hid_t group = H5Gcreate(file, groupname_lev.str().c_str(), H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
        ...
        for (int fe=0;fe<QL;fe++) {
	    if (fe < KK)  {
            Rest_pos[fe]   = new    int [n_dofs_fe_lev[fe][FEXLevel_c[fe]]*_mesh._elnodes[VV][fe]*_mesh._maxelxnode];
            Rest_val[fe]   = new double [n_dofs_fe_lev[fe][FEXLevel_c[fe]]*_mesh._elnodes[VV][fe]*_mesh._maxelxnode];
            mult_cols[fe]  = new    int [n_dofs_fe_lev[fe][FEXLevel_c[fe]]];
            for (uint i=0; i < n_dofs_fe_lev[fe][FEXLevel_c[fe]]*_mesh._elnodes[VV][fe]*_mesh._maxelxnode; i++) {
        ...
        for (int pr=0; pr<_mesh._NoSubdom; pr++) {
            for (int iel = _mesh._off_el[VV][pr*_NoLevels + Lev_c]; iel < _mesh._off_el[VV][pr*_NoLevels + Lev_c + 1]; iel++) {  //we are looping over the COARSE ELEMENTS,
                int   el_lib = _mesh._el_fm_libm[iel].second;
                int n_childs = _mesh._el_sto[el_lib]->_nch;
                for (int i_ch=0; i_ch< n_childs; i_ch++) {
                    int ch_el = _mesh._el_sto[el_lib]->_elchs[i_ch];

                    for (int fe=0;fe<QL;fe++) {
                        if (fe < KK)  {
                        for (uint k_fath=0; k_fath < _mesh._elnodes[VV][fe]; k_fath++) {

                            int op   = _mesh._nd_libm_fm[_mesh._el_sto[el_lib]->_elnds[k_fath]]; //coarse node (for rows)
                            int irow = _mesh._Qnode_fine_Qnode_lev[FEXLevel_c[fe]][op]; //coarse dof (for rows)    //the COARSE DOF determines the ROW

                            for (uint k_ch = 0; k_ch < _mesh._elnodes[VV][fe]; k_ch++) {
                                int ch_op = _mesh._nd_libm_fm[_mesh._el_sto[ch_el]->_elnds[k_ch]];   //child node (fine node) (for columns)
...
//                                mult_cols[LL][ g_indexL[ FELevel_c[LL] ][op]]=_elnodes[VV][LL];                   //TODO: what is this?
...
#else
                                double val = _AbstractFE[fe]->get_embedding_matrix(i_ch,k_ch,k_fath);   //the value AT THE FINE (CHILD) NODE of the SHAPE FUNCTION of the COARSE (FATHER) NODE
                                if ( fabs(val) > PseudoZero ) {

                                    int jcln     = _mesh._Qnode_fine_Qnode_lev[FEXLevel_f[fe]][ch_op];  //put the fine level, because it is the column level
                                    int indx0    = irow*_mesh._elnodes[VV][fe]*_mesh._maxelxnode;
			 ...
			 int sum_previous_sd_c = 0;
                            for (int sd=0;sd<pr;sd++) sum_previous_sd_c += _mesh._off_el[VV][sd*_NoLevels+Lev_c+1] - _mesh._off_el[VV][sd*_NoLevels+Lev_c];
                            int dof_pos_c = iel - _mesh._off_el[VV][pr*_NoLevels+Lev_c] + sum_previous_sd_c;
			 
			 int el_child_fm = _mesh._el_libm_fm[ch_el];
...
//Now the point is: with dof_extremes we
        ...
        for (int fe=0;fe<QL;fe++) {
            dof_extreme_at_fine_lev_sd[fe] = new int[_mesh._NoSubdom+1];
            dof_extreme_at_fine_lev_sd[fe][0] = 0;
            for (int idom=0;idom<_mesh._NoSubdom;idom++)  { 
	           if (fe  < KK)  dof_extreme_at_fine_lev_sd[fe][idom+1] = dof_extreme_at_fine_lev_sd[fe][idom] + ( _mesh._off_nd[fe][idom*_NoLevels + Lev_f +1] - _mesh._off_nd[fe][idom*_NoLevels]); 
              else if (fe == KK)  dof_extreme_at_fine_lev_sd[fe][idom+1] = dof_extreme_at_fine_lev_sd[fe][idom] + ( _mesh._off_el[VV][idom*_NoLevels + Lev_f +1] - _mesh._off_el[VV][idom*_NoLevels + Lev_f]);
           }
	}
...
//==============================
        ...
        for (int fe=0;fe<QL;fe++) {
	      ...
	      int delta_dof_at_coarse_lev_sd;
	           if (fe  < KK) delta_dof_at_coarse_lev_sd = (int) (_mesh._off_nd[fe][kdom*_NoLevels + Lev_c+1] - _mesh._off_nd[fe][kdom*_NoLevels]);
              else if (fe == KK) delta_dof_at_coarse_lev_sd =        _mesh._off_el[VV][kdom*_NoLevels + Lev_c+1] - _mesh._off_el[VV][kdom*_NoLevels + Lev_c];
	    
		for (int iki=0;iki < delta_dof_at_coarse_lev_sd; iki++) {
	           if (fe  < KK) {
                    for (uint k_ch=0;k_ch < mult_cols[fe][dof_row]; k_ch++) {
                        int k_fath=_mesh._elnodes[VV][fe]*_mesh._maxelxnode*dof_row+k_ch;
...
#ifdef DEFAULT_PRINT_INFO
    std::cout<< " EqnBase::compute_and_print_MGOps: compute_rest  \n";
#endif
