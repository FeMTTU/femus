...
GMVOutput::GMVOutput(MultiLevelProblem& ml_probl): Output(ml_probl)
{
...
{ 
  ...
  for (unsigned ig=igridr-1u; ig<igridn; ig++) {
    unsigned nvt_ig=_ml_probl._ml_msh->GetLevel(ig)->MetisOffset[index][_nprocs];
  ...
  for(unsigned ig=igridr-1u; ig<_gridn; ig++) {
    ...
    Mysol[ig]->init(_ml_probl._ml_msh->GetLevel(ig)->MetisOffset[index][_nprocs],_ml_probl._ml_msh->GetLevel(ig)->own_size[index][_iproc],true,AUTOMATIC);
  }
  ...
  for (int i=0; i<3; i++) {
    for (unsigned ig=igridr-1u; ig<igridn; ig++) {
      Mysol[ig]->matrix_mult(*_ml_probl._ml_msh->GetLevel(ig)->_coordinate->_Sol[i],*_ProlQitoQj[index][2][ig]);
	...
	for (unsigned ii=0; ii<nvt_ig; ii++) 
      ...
      if (_moving_mesh) {
	unsigned indDXDYDZ=_ml_probl.GetIndex(_moving_vars[i].c_str());
	Mysol[ig]->matrix_mult(*_ml_probl._solution[ig]->_Sol[indDXDYDZ],*_ProlQitoQj[index][_ml_probl.SolType[indDXDYDZ]][ig]);
	Mysol[ig]->localize_to_one(v_local,0);
	unsigned nvt_ig=_ml_probl._ml_msh->GetLevel(ig)->MetisOffset[index][_nprocs];      
	if(_iproc==0){ 
	  for (unsigned ii=0; ii<nvt_ig; ii++) 
  ...
  nel+=_ml_probl._ml_msh->GetLevel(igridn-1u)->GetElementNumber();
  ...
  for (unsigned ig=igridr-1u; ig<igridn; ig++) {
    for (unsigned ii=0; ii<_ml_probl._ml_msh->GetLevel(ig)->GetElementNumber(); ii++) {
      if ( ig==igridn-1u || 0==_ml_probl._ml_msh->GetLevel(ig)->el->GetRefinedElementIndex(ii)) {
        short unsigned ielt=_ml_probl._ml_msh->GetLevel(ig)->el->GetElementType(ii);
	...
	for(unsigned j=0;j<NVE[ielt][index];j++){
	  ...
	  unsigned jnode=_ml_probl._ml_msh->GetLevel(ig)->el->GetElementVertexIndex(ii,j)-1u;
	  unsigned jnode_Metis = _ml_probl._ml_msh->GetLevel(ig)->GetMetisDof(jnode,index);
    ...
    offset+=_ml_probl._ml_msh->GetLevel(ig)->MetisOffset[index][_nprocs];
  }
  ...
  for (unsigned ig=igridr-1u; ig<igridn; ig++) {
    for (unsigned ii=0; ii<_ml_probl._ml_msh->GetLevel(ig)->GetElementNumber(); ii++) {
      if ( ig==igridn-1u || 0==_ml_probl._ml_msh->GetLevel(ig)->el->GetRefinedElementIndex(ii)) {
	var_el[icount]=_ml_probl._ml_msh->GetLevel(ig)->el->GetElementGroup(ii);
        icount++;
      }
    }
  }
  ...
  if(_nprocs>=1){
    ...
    for (unsigned ig=igridr-1u; ig<igridn; ig++) {
      for (unsigned ii=0; ii<_ml_probl._ml_msh->GetLevel(ig)->GetElementNumber(); ii++) {
	if ( ig==igridn-1u || 0==_ml_probl._ml_msh->GetLevel(ig)->el->GetRefinedElementIndex(ii)) {
	  var_el[icount]=_ml_probl._ml_msh->GetLevel(ig)->epart[ii];
	  icount++;
	}
      }
    }
  ...
  for (unsigned i=0; i<_ml_probl.SolType.size(); i++) {
    for(int name=0;name<4;name++){
      if (name==0){
	sprintf(det,"%s %s","Sol",_ml_probl.SolName[i]);
      }
      else if (name==1){
	sprintf(det,"%s %s","Bdc",_ml_probl.SolName[i]);
      }
      else if (name==2){
	sprintf(det,"%s %s","Res",_ml_probl.SolName[i]);
      }
      else{
	sprintf(det,"%s %s","Eps",_ml_probl.SolName[i]);
      }
      if(name==0 || (debug && _ml_probl._solution[igridn-1u]->_ResEpsBdcFlag[i])){
	if (_ml_probl.SolType[i]<3) {  // **********  on the nodes **********
	  fout.write((char *)det,sizeof(char)*8);
	  fout.write((char *)&one,sizeof(unsigned));
	  for (unsigned ig=igridr-1u; ig<igridn; ig++) {
	    if (name==0){
	      Mysol[ig]->matrix_mult(*_ml_probl._solution[ig]->_Sol[i],*_ProlQitoQj[index][_ml_probl.SolType[i]][ig]);  
	    }
	    else if (name==1){
	      Mysol[ig]->matrix_mult(*_ml_probl._solution[ig]->_Bdc[i],*_ProlQitoQj[index][_ml_probl.SolType[i]][ig]);
	    }
	    else if (name==2){
	      Mysol[ig]->matrix_mult(*_ml_probl._solution[ig]->_Res[i],*_ProlQitoQj[index][_ml_probl.SolType[i]][ig]);
	    }
	    else{
	      Mysol[ig]->matrix_mult(*_ml_probl._solution[ig]->_Eps[i],*_ProlQitoQj[index][_ml_probl.SolType[i]][ig]);
	    }
	  ...
	  for (unsigned ig=igridr-1u; ig<igridn; ig++) {
	    std::vector<double> v_local;
	    if (name==0){
	      _ml_probl._solution[ig]->_Sol[i]->localize_to_one(v_local,0); 
	    }
	    else if (name==1){
	      _ml_probl._solution[ig]->_Bdc[i]->localize_to_one(v_local,0); 
	    }
	    else if (name==2){
	      _ml_probl._solution[ig]->_Res[i]->localize_to_one(v_local,0);
	    }
	    else{
	      _ml_probl._solution[ig]->_Eps[i]->localize_to_one(v_local,0);
	    }
	    for (unsigned ii=0; ii<_ml_probl._ml_msh->GetLevel(ig)->GetElementNumber(); ii++) {
	      if ( ig==igridn-1u || 0==_ml_probl._ml_msh->GetLevel(ig)->el->GetRefinedElementIndex(ii)) {
		unsigned iel_Metis = _ml_probl._ml_msh->GetLevel(ig)->GetMetisDof(ii,_ml_probl.SolType[i]);
