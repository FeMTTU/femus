...
template <class Base>
ImplicitRungeKutta<Base>::ImplicitRungeKutta(MultiLevelProblem& ml_probl,
...
/** Destructor. */
template <class Base>
ImplicitRungeKutta<Base>::~ImplicitRungeKutta()
{
  
}

template <class Base>
void ImplicitRungeKutta<Base>::UpdateAcceleration(const std::vector<std::string>& vel_vars, const std::vector<std::string>& acc_vars)
{
...
template <class Base>
void ImplicitRungeKutta<Base>::SetNewmarkParameters(const double gamma, const double delta)
{
  _gamma = gamma;
  _delta = delta;
