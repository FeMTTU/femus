...
//       }
        ...
        if(_SolType == 4) { //if piece_wise_linear derivatives
            for(int k = 0; k <  _nlag[0]; k++) {
                //coordinates of the coarse vertices with respect to the fine elements
	      if(i/8 == 1 ) {
                double xv = * ( linearElement->getX(  *(_pt_basis->getFine2CoarseVertexMapping(i%8) + k) ) + i/8 -1 );
                double yv = * ( linearElement->getX(  *(_pt_basis->getFine2CoarseVertexMapping((i+8)%8) + k) ) + (i+8)/8 -1 );
                double zv = * ( linearElement->getX(  *(_pt_basis->getFine2CoarseVertexMapping((i+16)%8) + k) ) + (i+16)/8 -1 );
                jac[1] += linearElement->eval_dphidx( linearElement->getIND(k), _X[i] ) * xv;
                jac[2] += linearElement->eval_dphidx( linearElement->getIND(k), _X[i+8] ) * yv;
                jac[3] += linearElement->eval_dphidx( linearElement->getIND(k), _X[i+16] ) * zv;
	      }
	      if(i/8 == 2 ) {
                double xv = * ( linearElement->getX(  *(_pt_basis->getFine2CoarseVertexMapping((i-8)%8) + k) ) + (i-8)/8 -1 );
                double yv = * ( linearElement->getX(  *(_pt_basis->getFine2CoarseVertexMapping(i%8) + k) ) + i/8 -1 );
                double zv = * ( linearElement->getX(  *(_pt_basis->getFine2CoarseVertexMapping((i+8)%8) + k) ) + (i+8)/8 -1 );
                jac[1] += linearElement->eval_dphidx( linearElement->getIND(k), _X[i-8] ) * xv;
                jac[2] += linearElement->eval_dphidx( linearElement->getIND(k), _X[i] ) * yv;
                jac[3] += linearElement->eval_dphidy( linearElement->getIND(k), _X[i+8] ) * zv;
	      }
	      if(i/8 == 3 ) {
                double xv = * ( linearElement->getX(  *(_pt_basis->getFine2CoarseVertexMapping((i-16)%8) + k) ) + (i-16)/8 -1 );
                double yv = * ( linearElement->getX(  *(_pt_basis->getFine2CoarseVertexMapping((i-8)%8) + k) ) + (i-8)/8 -1 );
                double zv = * ( linearElement->getX(  *(_pt_basis->getFine2CoarseVertexMapping(i%8) + k) ) + i/8 -1 );
                jac[1] += linearElement->eval_dphidz( linearElement->getIND(k), _X[i-16] ) * xv;
                jac[2] += linearElement->eval_dphidz( linearElement->getIND(k), _X[i-8] ) * yv;
...
//         }
        ...
        if(_SolType == 4) { //if piece_wise_linear derivatives
	      ...
	      if(i/8 == 2 ) {
                ...
                double yv = * ( linearElement->getX(  *(_pt_basis->getFine2CoarseVertexMapping(i%8) + k) ) + i/8 -1 );
                double zv = * ( linearElement->getX(  *(_pt_basis->getFine2CoarseVertexMapping((i+8)%8) + k) ) + (i+8)/8 -1 );
                jac[1] += linearElement->eval_dphidy( linearElement->getIND(k), _X[i-8] ) * xv;
