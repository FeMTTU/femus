...
extern "C" {
  ...
  // ------------------------------------------------------------------
  PetscErrorCode __Vankalibmesh_petsc_preconditioner_setup(void * ctx) {
    Preconditioner * preconditioner = static_cast<Preconditioner*>(ctx);
    preconditioner->init();
    return 0;
  }
  // ------------------------------------------------------------------------------
  PetscErrorCode __Vankalibmesh_petsc_preconditioner_apply(void *ctx, Vec x, Vec y)  {
    Preconditioner * preconditioner = static_cast<Preconditioner*>(ctx);
    PetscVector x_vec(x);
    PetscVector y_vec(y);
...
#else
  // ----------------------------------------------------------------
  PetscErrorCode __Vankalibmesh_petsc_preconditioner_setup(PC pc) {
    void *ctx;
  ...
  // --------------------------------------------------------------------------
  PetscErrorCode __Vankalibmesh_petsc_preconditioner_apply(PC pc, Vec x, Vec y) {
    void *ctx;
    PetscErrorCode ierr = PCShellGetContext(pc,&ctx);
    CHKERRQ(ierr);
    Preconditioner * preconditioner = static_cast<Preconditioner*>(ctx);
...
void VankaPetscLinearEquationSolver::SetElementBlockNumber(const unsigned & block_elemet_number){
  _num_elem_vanka_block = block_elemet_number;
  _indexai_init=0;
};

// *********************** for GMRES *********************************
clock_t VankaPetscLinearEquationSolver::BuildIndex(){
...
clock_t VankaPetscLinearEquationSolver::BuildAMSIndex(const vector <unsigned> &VankaIndex,
  ...
  while (test_end==0){
    ...
    int gel=_msh->IS_Mts2Gmt_elem_offset[iproc] + vb_index*_num_elem_vanka_block;
      // ***************** NODE/ELEMENT SERCH *******************
      
    for (int iel_mts=gel; iel_mts<gel+_num_elem_vanka_block && iel_mts< _msh->IS_Mts2Gmt_elem_offset[iproc+1]; iel_mts++) {
    ...
    if(gel+_num_elem_vanka_block <_msh->IS_Mts2Gmt_elem_offset[iproc+1] ) test_end=0;     
...
// // ********************************************************************************
// 
// std::pair< int, double> VankaPetscLinearEquationSolver::solve(const bool &ksp_clean) {
//     
//   clock_t SearchTime, AssemblyTime, SolveTime, UpdateTime;
//   int its;
//   double final_resid;
...
// ==============================================================
void VankaPetscLinearEquationSolver::init() {
  ...
  if (!this->initialized()) {
    ...
    if (this->_preconditioner) {
      ...
      PCShellSetSetUp(_pc,__Vankalibmesh_petsc_preconditioner_setup);
      PCShellSetApply(_pc,__Vankalibmesh_petsc_preconditioner_apply);
    }
  }
}
...
// =========================================================================
void VankaPetscLinearEquationSolver::get_residual_history(std::vector<double>& hist) {
  int ierr = 0;
  int its  = 0;
...
// ======================================================
double VankaPetscLinearEquationSolver::get_initial_residual() {
  int ierr = 0;
  int its  = 0;
  // Fill the residual history vector with the residual norms
  // Note that GetResidualHistory() does not copy any values, it
...
// =======================================================
void VankaPetscLinearEquationSolver::print_converged_reason() {
