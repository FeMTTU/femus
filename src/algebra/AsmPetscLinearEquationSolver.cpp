...
namespace femus {
  ...
  void AsmPetscLinearEquationSolver::BuildAMSIndex(const vector <unsigned>& variable_to_be_solved) {
    ...
    for(unsigned vb_index = 0; vb_index < _localIsIndex.size(); vb_index++) {
      ISCreateGeneral(MPI_COMM_SELF, _localIsIndex[vb_index].size(), &_localIsIndex[vb_index][0], PETSC_COPY_VALUES , &_localIs[vb_index]);
      ISCreateGeneral(MPI_COMM_SELF, _overlappingIsIndex[vb_index].size(), &_overlappingIsIndex[vb_index][0], PETSC_COPY_VALUES , &_overlappingIs[vb_index]);
    }
