...
bool Parameters::have_parameter (const std::string& name) const
{
  ...
  if (it != _values.end())
    ...
    if (static_cast<const Parameter<T>*>(it->second) != NULL)  // libmesh_cast_ptr
#endif // LIBMESH_HAVE_RTTI
      return true;
...
const T& Parameters::get (const std::string& name) const
{
  ...
  return static_cast<Parameter<T>*>(it->second)->get(); //libmesh_cast_ptr
}
...
T& Parameters::set (const std::string& name)
{
  ...
  return static_cast<Parameter<T>*>(_values[name])->set();  // libmesh_cast_ptr
}
