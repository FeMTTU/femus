...
void Marker::GetElement() {
    ...
    while(!elementHasBeenFound) {
		...
		double modulus = 0;
                for(unsigned i = 0; i < facePointNumber[ielType] - 1; i++) {
                    ...
                    modulus += sqrt(distanceIel);
                }
                modulus /= (facePointNumber[ielType] - 1);
            ...
            //if(iel != _mesh->_elementOffset[_iproc +1]) {
            if(iel != _mesh->_elementOffset[_iproc]) {
                std::cout << "start element= " << iel << std::endl;
                pointIsOutsideThisMesh = false;
                nextProc = _iproc;
                kel = iel;
            }
            else if (iel == _mesh->_elementOffset[_iproc]) {
                std::cout << " the marker is outside the domain " << std::endl;
                elementHasBeenFound = true;
            }
        }
