...
std::vector< double > Marker::InverseMappingTet(const unsigned &currentElem, const unsigned &solutionType,
   ...
   unsigned dim = 3;
    ...
    if(solutionType == 0) {
        ...
        for(int k = 0; k < dim; k++) {
            a[k][0] = xv[k][0];
            a[k][1] = - 3 * xv[k][0] - xv[k][2] + 4 * xv[k][4];
            a[k][2] = - 3 * xv[k][0] - xv[k][1] + 4 * xv[k][6];
            a[k][3] = - 3 * xv[k][0] - xv[k][3] + 4 * xv[k][7];
            a[k][4] = 7 * xv[k][0] + 27 * xv[k][10] + 3 * xv[k][1] + 3 * xv[k][2] - 8 * (2 * xv[k][4] + xv[k][5] + 2 * xv[k][6]);
            a[k][5] = 7 * xv[k][0] + 27 * xv[k][11] + 3 * xv[k][2] + 3 * xv[k][3] - 8 * (2 * (xv[k][4] + xv[k][7]) + xv[k][8]);
            a[k][6] = 7 * xv[k][0] - 8 * xv[k][9] + 27 * xv[k][13] + 3 * xv[k][1] + 3 * xv[k][3] - 16 * (xv[k][6] + xv[k][7]);
            a[k][7] = 2 * (xv[k][0] + xv[k][2] - 2 * xv[k][4]);
            a[k][8] = 2 * (xv[k][0] + xv[k][1] - 2 * xv[k][6]);
            a[k][9] = 2 * (xv[k][0] + xv[k][3] - 2 * xv[k][7]);
            a[k][10] = - 13 * xv[k][0] + 32 * xv[k][9] - 135 * xv[k][10] - 135 * xv[k][11] - 81 * xv[k][12] - 135 * xv[k][13] +
                       256 * xv[k][14] - 7 * xv[k][1] - 7 * xv[k][2] - 7 * xv[k][3] + 8 * (7 * xv[k][4] + 4 * xv[k][5] + 
                       7 * (xv[k][6] + xv[k][7]) + 4 * xv[k][8]);
            a[k][11] = - 3 * (xv[k][0] + 9 * xv[k][10] + xv[k][1] + xv[k][2] - 4 * (xv[k][4] + xv[k][5] + xv[k][6]));
            a[k][12] = - 3 * (xv[k][0] + 9 * xv[k][11] + xv[k][2] + xv[k][3] - 4 * (xv[k][4] + xv[k][7] + xv[k][8]));
            a[k][13] = - 3 * (xv[k][0] - 4 * xv[k][9] + 9 * xv[k][13] + xv[k][1] + xv[k][3] - 4 * (xv[k][6] + xv[k][7]));
...
;
	    ...
	    a[k][2] = - 3 * xv[k][11] - xv[k][14] + 4 * xv[k][17];  
	    a[k][3] = 0.5 * (- xv[k][0] + xv[k][3]);  
	    ...
	    a[k][12] = - 3 * (xv[k][12] + xv[k][13] + xv[k][14] - 4 * (xv[k][15] + xv[k][16] + xv[k][17]) + 9 * xv[k][20]);  
	    a[k][13] = - xv[k][0] - 2 * xv[k][9] - xv[k][1] + xv[k][3] + xv[k][4] + 2 * xv[k][6]; 
	    a[k][14] = - xv[k][0] - 2 * xv[k][11] - xv[k][2] + xv[k][3] + xv[k][5] + 2 * xv[k][8]; 
	    a[k][15] = 0.5 * (3 * xv[k][0] + 4 * xv[k][9] + 6 * xv[k][12] + 2 * xv[k][13] - 8 * xv[k][15] - xv[k][1] - 3 * xv[k][3] - xv[k][4] + 4 * xv[k][6]); 
	    a[k][16] = ; 
	    a[k][17] =; 
	    a[k][18] =; 
	    a[k][19] =; 
...
void Marker::InverseMappingTEST(std::vector < double > &x) {
    ...
    for(int solType = 0; solType <3; solType++) {
        ...
        for(unsigned i = 0; i < nDofs; i++) {
            ...
            for(unsigned k = 0; k < dim; k++) {
                xv[k][i] = (*_mesh->_topology->_Sol[k])(ielDof);     // global extraction and local storage for the element coordinates
            }
        }

        // This is the test
        std::vector < double > xp = InverseMappingTet(iel, solType, x);
        for(int k=0; k<dim; k++) {
            std::cout << " xp[" << k << "]= " << xp[k] << " , " <<  "xv[" << k << "][0] = " << xv[k][0] << " " ;
        }
        std::cout << std::endl;
