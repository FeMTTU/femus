...
const unsigned faceTriangleNodes[6][3][6][8][4] = { // elementType - solType - number of faces - number of triangles per faces - 3 vertices of the triangle (the first is counted twice)
    ...
    {{  {{0, 1, 15, 0}, {1, 4, 15, 1}, {4, 3, 15, 4}, {3, 0, 15, 3}},  //wedge
        ...
        {{2, 0, 17, 2}, {0, 3, 17, 0}, {3, 5, 17, 3}, {5, 2, 17, 2}},
        {{0, 2, 1, 0}},
        {{3, 4, 5, 3}}
    },
...
void Marker::GetElement(const bool &debug) {
    ...
    unsigned solType = 1; //TODO Get Soltype
    ...
    while(!elementHasBeenFound) {
        ...
        while(elementHasBeenFound + pointIsOutsideThisProcess + pointIsOutsideTheDomain == 0) {
            if(dim == 2) {
                nextElem[_iproc] = GetNextElement2D(iel, previousElem[_iproc], solType);
            }
            else if(dim == 3) {
                nextElem[_iproc] = GetNextElement3D(iel, previousElem[_iproc], solType);
            }
...
unsigned Marker::GetNextElement2D(const unsigned &currentElem, const unsigned &previousElem, const unsigned &solType) {
    ...
    if(xc[0]*xc[0] < epsilon2 && xc[1]*xc[1] < epsilon2) {
        ...
        for(unsigned k = 0; k < dim; k++) {
            xv[k].resize(facePointNumber[currentElementType][solType]);
        }

        for(unsigned i = 0; i < facePointNumber[currentElementType][solType]; i++) {
            unsigned inodeDof  = _mesh->GetSolutionDof(facePoints[currentElementType][solType][i], currentElem, 2);
        ...
        length /= facePointNumber[currentElementType][solType];
            ...
            for(unsigned i = 0 ; i < facePointNumber[currentElementType][solType] - 1; i++) {
                ...
                if(fabs(tBottom) < epsilon && tTop != 0) {
                    ...
                    if(t < 1) {   //if not, it means the point r is far away from the marker, and we don't want to go in that direction
                        ...
                        for(unsigned k = 0; k < dim; k++) {
                            xvr[k].resize(facePointNumber[currentElementType][solType]);
                        }

                        //now we have to determine if r is inside edge i
                        for(unsigned j = 0; j < facePointNumber[currentElementType][solType]; j++) {
                            for(unsigned k = 0; k < dim; k++) {
...
unsigned Marker::GetNextElement3D(const unsigned &currentElem, const unsigned &previousElem, const unsigned &solType) {
    ...
    if(xc[0]*xc[0] < epsilon2 && xc[1]*xc[1] < epsilon2 && xc[2]*xc[2] < epsilon2) {
        ...
        for(unsigned iface = 0; iface < _mesh->GetElementFaceNumber(currentElem); iface++) {
            ...
            for(unsigned itri = 0; itri < trianglesPerFace[currentElementType][solType][iface]; itri ++) {
                ...
                for(unsigned i = 0; i < 4; i++) {
                    unsigned itriDof  = _mesh->GetSolutionDof(faceTriangleNodes[currentElementType][solType][iface][itri][i], currentElem, 2);
