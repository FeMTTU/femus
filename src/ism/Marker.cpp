...
//           if(scalarCount == acePointNumber[ifaceType] - 1 && lineIntersection == true) {
  ...
  void Marker::InverseMappingTri(const unsigned &currentElem, const unsigned &solutionType,
                                 std::vector< double > &x) {
...
//         a[k][7] = - 3 * xv[k][0] - 3 * xv[k][1] - 3 * xv[k][2] + 12 * xv[k][3] +
//                   12 * xv[k][4] + 12 * xv[k][5] - 27 * xv[k][6];
      }
    }
...
// 
    ...
    std::cout << "element" << currentElem << " , " << "x coordinate = " << xp[0] << " , " << "y coordinate = " << xp[1] << "z coordinate = " << xp[2] << std::endl;
  ...
  void Marker::InverseMappingTEST(std::vector < double > &x) {
    unsigned dim = _mesh->GetDimension();
    std::vector < std::vector < double > > xv(dim);
    for(int solType = 0; solType < 1; solType++) {
      ...
      for(int iel = _mesh->_elementOffset[_iproc]; iel < _mesh->_elementOffset[_iproc + 1]; iel ++) {
        ...
        // This is the test
        InverseMappingHex(iel, solType, x);
	for(int k=0; k<dim; k++){
	  std::cout << xv[k][0] << " ";
	}
	std::cout << std::endl;
