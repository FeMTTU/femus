    ...
    for(unsigned iface = 0; iface <_mesh->GetElementFaceNumber(currentElem); iface++) {
		...
		std::cout<< "t= " << t <<std::endl;
		    ...
		    std::cout << "r[" << k << "] = " << r[k] <<std::endl; 
                ...
                if (fabs(t) < 1) {  //if not, it means the point r is farther away from the marker, and we don't want to go in that direction
...
//                 B = -(xv[2][2] - xv[2][0]) * (xv[0][1] - xv[0][0]) + (xv[0][2] - xv[0][0]) * (xv[2][1] - xv[2][0]);
//                 By0 = B * xv[1][0];
//                 Cz0 = C * xv[2][0];
//             }
//
...
//
//                         if(dim==3) {
//                             q0 = xv[1][i] * (xv[2][i] - xv[2][i + 1]) + xv[2][i] * (xv[1][i + 1] - xv[1][i]);
...
std::vector< double > Marker::InverseMappingQuad(const unsigned &currentElem, const unsigned &solutionType,
    ...
    while(!convergence) {
        ...
        if(solutionType > 0) {
            ...
            hessPhi[6][1][0] = hessPhi[6][0][1]= 2 * xi; // 2 x
            hessPhi[7][1][0] = hessPhi[7][0][1]= 2 * eta; // 2 y
        }
        if(solutionType > 1) {
