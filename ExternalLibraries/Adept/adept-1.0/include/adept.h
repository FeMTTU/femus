...
// once. The optimum value depends on platform. On modern Intel
// processors empirically the optimum appears to be 2 due to the SSE2
// instructions allowing two double multiplications to be carried out
// at once; presumably for single precision the optimum would be 4 due
// to the SSE instructions allowing four single-precision
// multiplications.
...
namespace adept {
  ...
  // current stack
  class Stack;
  extern __thread Stack* _stack_current_thread;
...
private:
  ...
  private:
    ...
    Statement* __restrict__ statement_ ;
    Real* __restrict__ gradient_;
    Real* __restrict__ multiplier_;
    Offset* __restrict__ offset_;
#endif
...
ADEPT_DEFINE_UNARY_FUNCTION(Log10,log10,0.434294481903252/a_.value())
...
ADEPT_DEFINE_UNARY_FUNCTION3(Tan,  tan, Real tmp = 1/cos(a_.value()), tmp * 2.0)
