...
GenCase::GenCase(Utils& mgutils_in,GeomEl& geomel_in,
    ...
    const uint mesh_ord = (uint) _utils._urtmap.get("mesh_ord");
    if (mesh_ord != 0) {
    ...
    _NoLevels = _utils._urtmap.get("nolevels");
...
void GenCase::GenerateCase()   {
    ...
    libMesh::Mesh* msh_coarse = new libMesh::Mesh( (libMesh::Parallel::Communicator) MPI_COMM_WORLD,_dimension);
...
void GenCase::GenerateCoarseMesh(Mesh* msh_coarse)  {
    ...
    case 1: {
        ...
        if ( _utils._urtmap.get("domain") == 0 ) {
...
//---Meshing -------
    ...
    case 0: {
        ...
        std::string f_mesh_read = _utils._files.get_frtmap().get("F_MESH_READ");
...
//===== store VV elem info
        ...
        for (int i=0;i<_n_elements_sum_levs[VV];i++)     _el_sto[i] = new ElemStoVol(_elnodes[VV][QQ],_dimension);
...
//===== store BB elem info
        ...
        for (int i=0; i<_n_elements_sum_levs[BB];i++)    _el_sto_b[i]= new ElemStoBdry(_elnodes[BB][QQ],_dimension);
...
//=====================================
void GenCase::ComputeElemOffsetsBySubdLevel() {
    _off_el = new int*[VB];
    int sum_el_vb[VB];
    for (int vb=0;vb < VB; vb++) {
...
//====================================
void GenCase::ComputeNodeOffsetsBySubdLevel()  {
...
// ==================================================================
void GenCase::ComputeNodeMapExtLevels() {
...
// linear coarse level
    ...
    //After _Qnode_fine_Qnode_lev is filled, I can fill the inverse map... can I do it on the fly?!   //Let me do it here now
      _Qnode_lev_Qnode_fine = new int *[_NoLevels+1];
     uint n_nodes_top=_NoNodesXLev[_NoLevels-1];

      for (uint ilev=0;ilev<=_NoLevels;ilev++) {
    _Qnode_lev_Qnode_fine[ilev] = new int [_NoNodesXLev[ilev]];
...
//   out << "[ <!ENTITY HeavyData \"mesh.h5\"> ] ";
    ...
    for (int vb=0;vb< VB;vb++) {
        ...
        for (int ilev=0; ilev<_NoLevels; ilev++) {
            ...
            _utils.PrintXDMFTopology(out,top_file.str(),hdf5_field.str(),_GeomEl.name[vb],_n_elements_lev_vb[vb][ilev],_n_elements_lev_vb[vb][ilev],_elnodes[vb][QQ]);
            std::ostringstream coord_lev; coord_lev << "_L" << ilev; 
	    _utils.PrintXDMFGeometry(out,top_file.str(),_nodes_name+"/COORD/X",coord_lev.str(),"X_Y_Z","Float",_NoNodesXLev[ilev],1);
            std::ostringstream pid_field;
            pid_field << "PID/PID_VB"<< vb <<"_L"<< ilev;
            _utils.PrintXDMFAttribute(out,top_file.str(),pid_field.str(),"PID","Scalar","Cell","Int",_n_elements_lev_vb[vb][ilev],1);
...
// =====================
    ...
    dimsf[0] = 4;
    dimsf[1] = 1;
    _utils.print_Ihdf5(file,"DFLS", dimsf,tdata);
    delete [] tdata;
...
// =====================
    ...
    dimsf[0] = VB;
    dimsf[1] = 1;
    _utils.print_Ihdf5(file,"ELNODES_VB", dimsf,ttype_FEM);
...
// nodes X lev
    ...
    dimsf[0] = _NoLevels+1;
    dimsf[1] = 1;
    _utils.print_Ihdf5(file, ndxlev.c_str(), dimsf,_NoNodesXLev);
...
// ++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    for (int ilev= 0;ilev < _NoLevels+1; ilev++) {
        ...
        _utils.print_Ihdf5(file,name.str(),dimsf,_Qnode_fine_Qnode_lev[ilev]);
    }
...
// ++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    for (int fe=0;fe < QL_NODES; fe++) {
        ...
        _utils.print_Ihdf5(file, namefe.str(),dimsf,_off_nd[fe]);
    }
...
// old        for (int inode=0; inode <_n_nodes_lev[l];inode++)  xcoord[inode] = _nod_coords[_nd_fm_libm[inode].second+kc*_n_nodes]; //the offset is fine
        ...
        _utils.print_Dhdf5(file,name.str(), dimsf,xcoord);
    }
...
// ===========================================
    ...
    for (int lev=0;lev<_NoLevels; lev++)  {
        ...
        dimsf[1] = 1;
        _utils.print_Ihdf5(file,bname.str(), dimsf,_el_child_to_fath[lev]);
    }
...
void GenCase::PrintElemVB(hid_t file,const uint vb , int* nd_libm_fm , ElemStoBase** el_sto_in, std::vector<std::pair<int,int> > el_fm_libm_in ) {
    ...
    _utils.print_Ihdf5(file,(elems_fem_vb + "/NDOF_FO_F1"), dimsf,ndofm);
    // NoElements ------------------------------------
    dimsf[0] = _NoLevels;
    dimsf[1] = 1;
    _utils.print_Ihdf5(file,(elems_fem_vb + "/NExLEV"), dimsf,_n_elements_lev_vb[vb]);
    // offset
    dimsf[0] = _NoSubdom*_NoLevels+1;
    dimsf[1] = 1;
    _utils.print_Ihdf5(file,(elems_fem_vb + "/OFF_EL"), dimsf,_off_el[vb]);
    ...
    dimsf[1] = 1;
    _utils.print_Ihdf5(file,(elems_fem_vb + "/CONN"), dimsf,tempconn);
    ...
    for (int ilev=0;ilev <_NoLevels; ilev++) {
        ...
        _utils.print_Ihdf5(file,name.str(), dimsf,conn_lev);
        //clean
        delete []conn_lev;
...
void GenCase::PrintSubdomFlagOnQuadrCells(const int vb, const int Level, std::string filename) const {

    if (libMesh::global_processor_id() == 0/*_iproc==0*/)   {
        ...
        _utils.print_Ihdf5(file_id,name.str(),dimsf,ucoord);
...
void GenCase::PrintOneVarMatrixHDF5(std::string name, std::string groupname, int** n_dofs_lev_fe,int count,
    ...
    _utils.print_Ihdf5(file,name0.str().c_str(),dimsf,rowcln);

    //===== POS =======
    std::ostringstream name1;
    name1 << groupname << "/"  << "POS" << fe_couple.str();
    dimsf[0]=count;
    _utils.print_Ihdf5(file,name1.str().c_str(),dimsf,Mat);

    //===== LEN =======
    std::ostringstream name2;
    name2 << groupname << "/"  << "LEN" << fe_couple.str();
    dimsf[0]=rowcln[0]+1;
    _utils.print_Ihdf5(file,name2.str().c_str(),dimsf,len);

    //==== OFFLEN ========
    std::ostringstream name3;
    name3 << groupname << "/"  << "OFFLEN" << fe_couple.str();
    dimsf[0]= rowcln[0]+1;
    _utils.print_Ihdf5(file,name3.str().c_str(),dimsf,len_off);
...
void GenCase::PrintOneVarMGOperatorHDF5(std::string filename, std::string groupname, int* n_dofs_lev, int count, int* Op_pos,double* Op_val,int* len,int* len_off, int FELevel_row, int FELevel_col, int fe) {
    ...
    dimsf[0] = count;
    _utils.print_Ihdf5(file,name1.str().c_str(),dimsf,Op_pos);
    //===== VAL =======
    std::ostringstream name1b;
    name1b << groupname << "/" << "VAL" << fe_family.str();
    dimsf[0] = count;
    _utils.print_Dhdf5(file,name1b.str().c_str(),dimsf,Op_val);
    //===== LEN =======
    std::ostringstream name2;
   ...
   for (uint i= 0; i< rowcln[0]; i++) { 
    ...
    _utils.print_Ihdf5(file,name4.str().c_str(),dimsf,&mat_op[0][0]);
...
//===========================    
   ...
   for (uint i= 0; i< rowcln[0]; i++) { 
    ...
    _utils.print_Dhdf5(file,name5.str().c_str(),dimsf,&mat[0][0]);
  ...
  double   PseudoZero = 1.e-8;
    ...
    hid_t group = H5Gcreate(file, groupname_lev.str().c_str(), H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
	    ...
	    if (fe < KK)  {
            ...
            Rest_pos[fe]   = new    int [ n_dofs_fe_lev[fe][FEXLevel_c[fe]]*4*(_dimension-1) /**_elnodes[VV][fe]*/]; //here I have to put the number of childs
            Rest_val[fe]   = new double [ n_dofs_fe_lev[fe][FEXLevel_c[fe]]*4*(_dimension-1) /**_elnodes[VV][fe]*/];
            mult_cols[fe]  = new    int [ n_dofs_fe_lev[fe][FEXLevel_c[fe]] ];  //TODO unused
            for (uint i=0; i < n_dofs_fe_lev[fe][FEXLevel_c[fe]]*4*(_dimension-1) /**_elnodes[VV][fe]*/; i++) {
                Rest_pos[fe][i] = NegativeOneFlag;
                Rest_val[fe][i] = 0.;
               }
            for (int im = 0; im < n_dofs_fe_lev[fe][FEXLevel_c[fe]]; im++)     mult_cols[fe][im]=0;
...
//                                mult_cols[LL][ g_indexL[ FELevel_c[LL] ][op]]=_elnodes[VV][LL];                   //TODO: what is this?
			 ...
			 int sum_previous_sd = 0;
                            ...
                            int dof_pos_f = el_child_fm - _off_el[VV][pr*_NoLevels+Lev_f] + sum_previous_sd;
                             ...
                             Rest_pos[fe][ dof_pos_c*4*(_dimension-1) + i_ch ] = dof_pos_f;
                             Rest_val[fe][ dof_pos_c*4*(_dimension-1) + i_ch ] = 1;
...
//==============================
		...
		for (int iki=0;iki < delta_dof_at_coarse_lev_sd; iki++) {
		   ...
		   for (uint k_ch = 0; k_ch < 4*(_dimension-1); k_ch++) {
