#ifndef __parallelM_h__
#define __parallelM_h__


#include "FemusExtLib_conf.hpp"   //this is only for HAVE_MPI

//try to check the uses of HAVE_MPI and LIBMESH_HAVE_MPI
// and HAVE_MPIM

//The only other macro variable that I see here is NDEBUG:
//where does it get it? from the shell?
//i didnt find its definition in the project files
//here NDEBUG does not arrive
...
#ifndef NDEBUG
#define parallel_onlyM() do {assert(ParallelM::verify(std::string(__FILE__))); assert(ParallelM::verify(__LINE__)); } while (0)
#else
#define parallel_onlyM()
#endif

/**
 * The ParallelM namespace is for wrapper functions
...
public:
  ...
  DataType & operator = (const DataType &other)     { _datatype = other._datatype; return *this; }
  DataType & operator = (const data_type &type)    { _datatype = type; return *this; }
  operator const data_type & () const    { return _datatype; }
...
public:
  ...
  operator status const *()const {return &_status; }
//     operator status & () { return _status; }
...
public:
  ...
  Request & operator = (const Request &other)  { _request = other._request; return *this; }
  Request & operator = (const request &r) { _request = r; return *this; }
  ...
  operator const request & () const    { return _request; }
  operator request & ()  { return _request; }
...
//     MPI_Comm_rank (MPI_COMM_WORLD,&iproc);
  if (n_proc > 1)      {
    ...
    ParallelM::min(tempmin);
    ParallelM::max(tempmax);
    bool verified = (r == tempmin) &&
                    (r == tempmax);
    ParallelM::min(verified);
    return verified;
  }
  return true;
}
...
template <typename T> inline void min(T &r)  {
  ...
  MPI_Comm_size(MPI_COMM_WORLD,&n_proc);
  if (n_proc > 1)      {
    // START_LOG("min()", "ParallelM");
    T temp = r;
    MPI_Allreduce(&temp, &r, 1, datatype<T>(),
                  MPI_MIN,MPI_COMM_WORLD);

    // STOP_LOG("min()", "ParallelM");
  }
}
...
inline status wait(request &r) {
  // START_LOG("wait()", "ParallelM");

  status status;

  MPI_Wait(&r, &status);

  // STOP_LOG("wait()", "ParallelM");
...
inline void send_receive(const unsigned int dest_processor_id,
                         ...
                         std::vector<T> &recv,
                         const DataType &type) {
  // START_LOG("send_receive()", "ParallelM");
  int iproc;
MPI_Comm_size (MPI_COMM_WORLD,&iproc);
  ...
  if (dest_processor_id   == iproc &&
    ...
    recv = send;
    // STOP_LOG("send_receive()", "ParallelM");
    return;
  }

  ParallelM::request request;

  ParallelM::nonblocking_send(dest_processor_id,
                              send,
                              type,
                              request,
                              /* tag = */ 321);

  ParallelM::receive(source_processor_id,
                     recv,
                     type,
                     /* tag = */ 321);

  ParallelM::wait(request);
...
inline void gather(const unsigned int root_id,
  ...
  MPI_Comm_size(MPI_COMM_WORLD,&n_proc);
  MPI_Comm_rank(MPI_COMM_WORLD,&iproc);
  if (n_proc == 1)    {assert( ((uint)iproc) == root_id);return;   }

  std::vector<int> sendlengths(n_proc, 0), displacements(n_proc, 0);
  const int mysize = r.size();
  ParallelM::allgather(mysize, sendlengths);
...
inline void broadcast(T &data, const unsigned int root_id) {
  ...
  MPI_Comm_size(MPI_COMM_WORLD,&n_proc);
  MPI_Comm_rank(MPI_COMM_WORLD,&iproc);
  if (n_proc == 1)  {assert(iproc == root_id);return; }
  // START_LOG("broadcast()", "ParallelM");
  ...
  assert(ierr == MPI_SUCCESS);
  // STOP_LOG("broadcast()", "ParallelM");
}


// ========================================================
// template <>   // TODO WAS JUST THIS THE PROBLEM?
//so what does instantiated from here mean?
//What does the compiler want to tell me?
//Something like:finally at this point i understood the difference
// between the two broadcast functions and i can go ahead?
//Anyway, put <typename T> instead of <> !
//In libmesh parallel.h, things are written in the same way,
//so we should have the same warning when compiling libmesh as well...
//The "instantiated from here" warning is given only in DEBUG mode.
//Are we sure that things are alright with this one here?
//Is this function a template specialization of the previous one, or not?
//In order to write a template specialization, shouldnt we write it in 
//a different manner?
//In the case of template specialization, can we always be sure that
//we are calling the correct functionand not the general one?
...
inline void broadcast(std::string &data, const unsigned int root_id_in) {
 ...
 unsigned int root_id=root_id_in;
  ...
  ParallelM::broadcast(data_size,root_id);
  std::vector<char> data_c(data_size);
  std::string orig(data);

  if (iproc == root_id)  for (unsigned int i=0; i<data.size(); i++)	data_c[i] = data[i];

  ParallelM::broadcast(data_c,root_id);

  data.clear(); data.reserve(data_c.size());
  for (unsigned int i=0; i<data_c.size(); i++)  data.push_back(data_c[i]);

  if ((uint)iproc == root_id)   assert(data == orig);
  // STOP_LOG("broadcast()", "ParallelM");
}
...
// ==================================
template <typename T>  inline bool verify(const T &) { return true; }
// =============================
...
inline status probe(const int,  const int) { abort(); status status; return status; }
...
inline void send(const unsigned int, std::vector<T> &,
                 const DataType &,  const int) { abort(); }
...
template <typename T>
inline void send(const unsigned int,  std::vector<T> &,
                 const DataType &,   request &,  const int) { abort(); }
...
inline Status receive(const int,   std::vector<T> &,
                      const DataType &, const int) { abort(); return Status(); }
